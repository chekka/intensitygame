<?php
/**
 * Booster Plus for WooCommerce - Shortcodes - Products Add Form
 *
 * @version 5.6.1
 * @since  1.0.0
 * @author  Pluggabl LLC.
...
class WCJ_Products_Add_Form_Shortcodes extends WCJ_Shortcodes {
	...
	function __construct() {
		...
		$this->the_atts = array(
			...
			'image_gallery_enabled'  => apply_filters( 'booster_option', 'no', wcj_get_option( 'wcj_product_by_user_image_gallery_enabled', 'no' ) ),
			...
			'image_gallery_required'         => apply_filters( 'booster_option', 'no', wcj_get_option( 'wcj_product_by_user_image_gallery_required', 'no' ) ),
	...
	function wc_add_new_product( $args, $shortcode_atts ) {
		...
		if ( 0 != $product_id ) {
			...
			// Image Gallery
		if ( isset($args['image_gallery']['name'][0]) && "" != $args['image_gallery']['name'][0] ) {
            $files = $_FILES['wcj_add_new_product_gallery_image'];
            foreach ($files['name'] as $key => $value){
                ...
                if ($files['name'][$key]){
	                ...
	                $_FILES = array('wcj_add_new_product_gallery_image' => $file);
	                $i=1;
	...
	function validate_args( $args, $shortcode_atts ) {
		...
		$fields = array(
			...
			'image_gallery' => __( 'Image Gallery', 'woocommerce-jetpack' ),
		...
		foreach ( $fields as $field_id => $field_desc ) {
			if (  'yes' === $shortcode_atts[ $field_id . '_enabled' ] && 'yes' === $shortcode_atts[ $field_id . '_required' ] ) {
				...
				if ( 'image' === $field_id && ( '' == $args[ $field_id ] || ! isset( $args[ $field_id ]['tmp_name'] ) || '' == $args[ $field_id ]['tmp_name'] ) ) {
					$is_missing = true;
				}elseif ( 'image_gallery' === $field_id && ( '' == $args[ $field_id ] || ! isset( $args[ $field_id ]['tmp_name'] ) || '' == $args[ $field_id ]['tmp_name'] ) ) {
					$is_missing = true;
	...
	function wcj_product_add_new( $atts ) {
		...
		$args = array(
			...
			'image_gallery'             => isset( $_FILES['wcj_add_new_product_gallery_image'] )           ? $_FILES['wcj_add_new_product_gallery_image']           : '',
		);
		for ( $i = 1; $i <= $this->the_atts['custom_taxonomies_total']; $i++ ) {
		...
		if(isset($_REQUEST['gallary_image_id']))
			{
							
		     wp_delete_post( $_REQUEST['gallary_image_id'], true );
		...
		if ( 'yes' === $atts['image_enabled'] ) {
			...
			if ( 0 != $atts['product_id'] ) {
				...
				$the_field = ( 0 == get_post_thumbnail_id( $atts['product_id'] ) ) ?
					$new_image_field :
		...
		if ( 'yes' === $atts['image_gallery_enabled'] ) {
			$required_html           = ( 'yes' === $atts['image_gallery_required'] ) ? ' required' : '';
			$required_mark_html      = ( 'yes' === $atts['image_gallery_required'] ) ? $required_mark_html_template : '';
			$new_image_field_gallary = '<input' . $required_html . ' type="file" id="wcj_add_new_product_gallery_image" name="wcj_add_new_product_gallery_image[]" multiple="multiple" accept="image/*">';
			if ( 0 != $atts['product_id'] ) {
                 ...
                 $gallary_images     = "";
					        ...
					        foreach( $attachment_ids as $attachment_id ) 
					        {
					          ...
					          // Display Image instead of URL
					           $gallary_images     .= wp_get_attachment_image($attachment_id, 'full');
					           $check_meta_data     = get_metadata( 'post', $attachment_id, '_wp_attached_file',true);
							         if($check_meta_data  != "")
							         {

							                    $the_gallary_field.= ( '' == $gallary_images ) ?
							                    '':
												'<a href="' . add_query_arg( 'gallary_image_id', $attachment_id ) . '" onclick="return confirm(\'' .
												__( 'Are you sure?', 'woocommerce-jetpack' ) . '\')">' . __( 'Delete', 'woocommerce-jetpack' ) . '</a><br>' .
												wp_get_attachment_image( $attachment_id, array( 50, 50 ) , array( 'class' => 'alignleft' ) );
							         }
					        }
     
            } else {
				$the_gallary_field = $new_image_field_gallary;
			}
			if(@$gallary_images == "")
			{
				$the_gallary_field = '<input' . $required_html . ' type="file" id="wcj_add_new_product_gallery_image" name="wcj_add_new_product_gallery_image[]" multiple="multiple" accept="image/*">';
			}
			$table_data[] = array(
				'<label for="wcj_add_new_product_gallery_image">' . __( 'Image Gallary', 'woocommerce-jetpack' ) . $required_mark_html . '</label>',
				$the_gallary_field
			);
		}
