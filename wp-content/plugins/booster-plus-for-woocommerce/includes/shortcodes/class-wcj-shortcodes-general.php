	...
	private function get_currency_selector($atts, $content, $type = 'select')
		{
			...
			$shortcode_currencies = $this->get_shortcode_currencies($atts); // selected currency
			$currencies = get_woocommerce_currencies(); // all currency list
			$geoData   = WC_Geolocation::geolocate_ip();
			$country_code_by_ip = ($geoData['country']) ? $geoData['country'] : WC()->countries->get_base_country();
			$country_currency_by_ip = $all_country_currency[$country_code_by_ip] ?? $all_country_currency[WC()->countries->get_base_country()];
			$defualt_currency  = get_woocommerce_currency();
			$ip_addrs = wcj_session_get('wcj-ip');
			if ($ip_addrs == $country_currency_by_ip) {
				$i = wcj_session_get('wcj-i-impact', $i);
			} else {
				if( isset( $_SESSION['wcj-ip'] ) ){
					unset($_SESSION['wcj-ip']);
				}
				$i = 1;
				$ip_addrs = wcj_session_get('wcj-ip');
			}
			...
			if (null !== ($session_value = wcj_session_get('wcj-currency'))) {
				...
				if ('yes' == wcj_get_option('wcj_multicurrency_set_customer_ip_currency', 'no')) {

					if (in_array($country_currency_by_ip, $shortcode_currencies) && $i == 1) {
						$selected_currency =  $country_currency_by_ip;
						wcj_session_set('wcj-currency', $country_currency_by_ip);
						$ip_addrs = $country_currency_by_ip;
						$ip_addrs = wcj_session_set('wcj-ip', $ip_addrs);
						$i = $i + 1;
						wcj_session_set('wcj-i-impact', $i);
					} elseif (in_array($defualt_currency, $shortcode_currencies) && $i != 1) {
						$selected_currency =  $defualt_currency;
						wcj_session_set('wcj-currency', $defualt_currency);
					} elseif ($i == 1) {

						$key = 1;
						$defualt_currency_key = wcj_get_option('wcj_multicurrency_default_currency', '');
						$defualt_currency_array_position = $defualt_currency_key - $key;
						wcj_session_set('wcj-currency', $shortcode_currencies[$defualt_currency_array_position]);
						$selected_currency =  $shortcode_currencies[$defualt_currency_array_position];
						$ip_addrs = $country_currency_by_ip;
						$ip_addrs = wcj_session_set('wcj-ip', $ip_addrs);
						$i = $i + 1;
						wcj_session_set('wcj-i-impact', $i);
