...
add_filter( 'wcj_modules', function ( $modules_cats ) {
    ...
    if( wcj_is_module_enabled( 'modules_by_user_roles' ) ) {
        ...
        foreach ( $modules_all_cats as $key => $modules_cat ) {
            foreach ( $modules_cat as $cat_keys => $value ) {
                if( $key == "dashboard" || $cat_keys != "all_cat_ids" ){
                    continue;
                }
                $modules_cats[$key][ "all_cat_ids" ] = array();
                foreach ( $value as $module ) {
                    if( in_array( $module , $all_cat_ids ) ) {
    ...
    add_filter( 'wcj_modules_loaded' ,function( $modules ) use( $all_cat_ids ){
        $modules = array_filter( $modules, function ( $item ) use( $all_cat_ids ) {
            return in_array( $item, $all_cat_ids );
        }, ARRAY_FILTER_USE_KEY );
        return $modules;
    }, 10, 2);
    return $modules_cats;
