<?php
/**
 * Booster Elite for WooCommerce - Settings Meta Box - Wholesale Price
 *
 * @version 5.6.4
 * @since  3.2.4
 * @author  Pluggabl LLC.
...
$wholesale_price_levels_num = apply_filters( 'booster_option', 1, get_post_meta( $product_id, '_wcj_wholesale_price_levels_number', true ) );
	...
	$options = array_merge(
		...
		array(
			...
			array(
				...
				'default' => 0,
				'type'    => 'price',
				'title'   => __( 'Level', 'woocommerce-jetpack' ) . ' #' . $i . ' ' . ( 'price_directly' === get_post_meta( $product_id, '_wcj_wholesale_price_discount_type', true ) ? __( 'Price', 'woocommerce-jetpack' ) : __( 'Discount', 'woocommerce-jetpack' ) ),
			),
		)
	);
}
...
if ( ! empty( $user_roles ) ) {
	foreach ( $user_roles as $user_role_key ) {
		...
		$wholesale_price_levels = apply_filters( 'booster_option', 1, get_post_meta( $product_id, '_wcj_wholesale_price_levels_number_' . $user_role_key, true ) );
		for ( $i = 1; $i <= $wholesale_price_levels; $i++ ) {
			$options = array_merge(
				$options,
				array(
					array(
