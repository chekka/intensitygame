	...
	for ( $i = 1; $i <= $total_number; $i++ ) {
		...
		$currencies    = array_merge(
			...
			array(
				array(
					...
					'meta_name'  => '_wcj_multicurrency_per_product_regular_price_' . $currency_code,
				),
				array(
					...
					'desc'       => $desc,
					'product_id' => $product_id,
					'meta_name'  => '_wcj_multicurrency_per_product_sale_price_' . $currency_code,
				),
			)
		);
		if ( 'yes' === wcj_get_option( 'wcj_multicurrency_per_product_make_empty', 'no' ) ) {
			$currencies[] = array(
				...
				'desc'       => $desc,
				'product_id' => $product_id,
				'meta_name'  => '_wcj_multicurrency_per_product_make_empty_' . $currency_code,
			);
		}
	}
}
