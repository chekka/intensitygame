...
class WCJ_One_page_checkout extends WCJ_Module {
	...
	function __construct() {
		...
		if ( $this->is_enabled() ) {
			...
			if ( wcj_is_frontend() ) {
                ...
                add_action( 'woocommerce_after_single_product_summary', array( $this, 'wcj_opc_single_product_checkout' ), get_option('wcj_one_page_checkout_hooks_priority') );
	 ...
	 * @since  1.0.0
	 */
		function wcj_expired_empty_cart_error( $error ) {

		if ( $error == sprintf( __( 'Sorry, your session has expired. <a href="%s">Return to homepage &rarr;</a>', 'woocommerce-jetpack' ), home_url() ) ) {
	 ...
	 * @since  1.0.0
	 */
	function wcj_allow_expired_session() {
		if ( WC()->cart->is_empty() ) {
