<?php
/**
 * Booster Plus for WooCommerce - Module - Custom Price Labels
 *
 * @version 5.6.4
 * @author  Pluggabl LLC.
	...
	class WCJ_Price_Labels extends WCJ_Module {
		...
		public function create_price_label_meta_box() {
			...
			foreach ( $this->custom_tab_sections as $custom_tab_section ) {
				...
				echo '<td style="width:25%;">' . wp_kses_post( $disabled_if_no_plus ) . '</td>';
			}
			echo '</tr>';
			echo '</table>';
		...
		public function custom_price( $price, $product ) {
			...
			if ( WCJ_IS_WC_VERSION_BELOW_3 && 'woocommerce_get_price_html' === $current_filter_name && ! in_array( $_product_type, apply_filters( 'wcj_price_labels_woocommerce_get_price_html_allowed_post_types', array( 'booking' ), $_product_type ), true ) ) {
				return $price;
			}
			...
			if ( $do_apply_global ) {
				...
				$text_instead = wcj_get_option( 'wcj_global_price_labels_instead_text', '' );
				if ( '' !== ( $text_instead ) ) {
					$price = $text_instead;
				}
			}
			...
			if ( 'yes' === wcj_get_option( 'wcj_local_price_labels_enabled', 'yes' ) ) {
				foreach ( $this->custom_tab_sections as $custom_tab_section ) {
					...
					if ( 'on' === $labels_array['variation_enabled'] ) {
						...
						( 'on' === $labels_array['variation_variable'] && in_array(
							$current_filter_name,
							array(
								'woocommerce_variable_empty_price_html',
								'woocommerce_variable_free_price_html',
						...
						( 'on' === $labels_array['variation_variation'] && in_array(
							$current_filter_name,
							array(
								'woocommerce_variation_empty_price_html',
								'woocommerce_variation_free_price_html',
