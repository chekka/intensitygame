...
final class WFACP_admin {
	...
	protected function __construct() {
		...
		// Embed form code
		add_action( 'admin_menu', [ $this, 'save_meta_box_value' ], 91 );
		add_action( 'add_meta_boxes_page', [ $this, 'add_meta_boxes_for_make_page_as_checkout' ], 10, 2 );
		...
		add_filter( "get_pages", [ $this, 'add_pages_to_front_page_options' ], 15 );
	...
	public function maybe_register_breadcrumbs() {
		if ( WFACP_Common::is_load_admin_assets( 'builder' ) ) {
			...
			BWF_Admin_Breadcrumbs::register_node( array(
				'text'  => WFACP_Common::get_page_name(),
				'link'  => '',
	...
	public function global_dependency_messages() {
		...
		if ( wc_shipping_enabled() ) {
			...
			$msg               = __( sprintf( 'Your store has <a href="%s">shipping location</a> enabled. Depending upon shipping method configuration, checkout may need  "Order Summary" field. Please drag "Order Summary" field to place in form. ', $shipping_location, $shipping_methods ), 'woofunnels-aero-checkout' );
			$aero_messages[]   = [
				'message'     => $msg,
				'id'          => 'order_summary',
				'show'        => 'yes',
...
#order_data #wfacp_admin_advanced_field span.wfacp_radio_options_group:after{    clear: both;} </style>";
	...
	public function save_meta_box_value() {
		...
		$wpnonce = filter_input( INPUT_POST, '_wpnonce', FILTER_UNSAFE_RAW );
		$id      = filter_input( INPUT_POST, 'post_ID', FILTER_UNSAFE_RAW );
		if ( is_null( $wpnonce ) || is_null( $id ) ) {
			return;
		}

		if ( wp_verify_nonce( $wpnonce, 'update-post_' . $id ) && isset( $_REQUEST['post_type'] ) && 'page' == $_REQUEST['post_type'] && isset( $_REQUEST['post_ID'] ) ) {
			if ( isset( $_REQUEST['aero-checkout-checkbox'] ) ) {
				update_post_meta( $id, '_is_aero_checkout_page', 1 );
			} else {
				delete_post_meta( $id, '_is_aero_checkout_page' );
			}
		}
	...
	public function add_meta_boxes_for_make_page_as_checkout() {
		add_meta_box( 'woofunnels-aero-checkout', __( 'FunnelKit Checkout', 'woofunnels-aero-checkout' ), [
			$this,
			'render_my_meta_box'
		], 'page', 'side', 'default' );
	}
	...
	public function render_my_meta_box() {
		...
		echo "<input type='checkbox' id='aero-checkout-checkbox' name='aero-checkout-checkbox' " . $is_checked . "><label  for='aero-checkout-checkbox'>" . __( 'Make this page as a checkout page', 'woofunnels-aero-checkout' ) . '</label> <p style="
    font-size: 12px;
    margin-top: 5px;
    font-style: italic;
">' . __( 'Note: Only check this setting if you are embedding checkout shortcode on Pages/Posts', 'woofunnels-aero-checkout' ) . '</p>';
	...
	public function render_shortcode_meta_box() {
		$id                  = WFACP_Common::get_id();
		$normal              = "[wfacp_forms]";
		$mini_cart_shortcode = "[wfacp_mini_cart]";
		...
		if ( $id > 0 ) {
            ...
            <div class="wfacp_shortcode">
                ...
                <div class="wfacp_shortcode_inner">
                    <div class="wfacp_description">
                        <label for='wfacp_shortcode_normal'><?php _e( 'Mini Cart Shortcode', 'woofunnels-aero-checkout' ) ?></label>
                        <input type="text" readonly="readonly" id='wfacp_shorcode_normal' style="width: 100%;" value="<?php echo $mini_cart_shortcode ?>">
                    </div>
                    <a href="javascript:void(0)" class="wfacp_copy_text">
                        <svg fill="#0073aa" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="20" height="20">
                            <path d="M 18.5 5 C 15.480226 5 13 7.4802259 13 10.5 L 13 32.5 C 13 35.519774 15.480226 38 18.5 38 L 34.5 38 C 37.519774 38 40 35.519774 40 32.5 L 40 10.5 C 40 7.4802259 37.519774 5 34.5 5 L 18.5 5 z M 18.5 8 L 34.5 8 C 35.898226 8 37 9.1017741 37 10.5 L 37 32.5 C 37 33.898226 35.898226 35 34.5 35 L 18.5 35 C 17.101774 35 16 33.898226 16 32.5 L 16 10.5 C 16 9.1017741 17.101774 8 18.5 8 z M 11 10 L 9.78125 10.8125 C 8.66825 11.5545 8 12.803625 8 14.140625 L 8 33.5 C 8 38.747 12.253 43 17.5 43 L 30.859375 43 C 32.197375 43 33.4465 42.33175 34.1875 41.21875 L 35 40 L 17.5 40 C 13.91 40 11 37.09 11 33.5 L 11 10 z"></path>
	...
	public function arrange_billing_fields( $options ) {
		...
		$addressOrder = wc_clean( $_POST['address_order'] );
	...
	function get_advanced_field() {
		if ( isset( $_REQUEST['post'] ) && $_REQUEST['post'] > 0 ) {
			$order_id = filter_input( INPUT_GET, 'post', FILTER_UNSAFE_RAW );
	...
	public function add_pages_to_front_page_options( $args ) {
		...
		if ( 'options-reading.php' === $pagenow ) {
			$pages = WFACP_Common::get_saved_pages();
			foreach ( $pages as $page ) {
				$post             = get_post( $page['ID'] );
				$post->post_title .= " (#" . $post->ID . " - AeroCheckout)";
				$args[]           = $post;
			}
		}
	...
	public function admin_footer_text( $footer_text ) {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'wfacp' ) {
			...
			$footer_text = __( 'Thanks for creating with FunnelKit. Need help? <a href="https://funnelkit.com/support/?utm_source=WordPress&utm_medium=Footer+Checkout&utm_campaign=Lite+Plugin" target="_blank">Contact Support</a>', 'funnel-builder' );
		}
