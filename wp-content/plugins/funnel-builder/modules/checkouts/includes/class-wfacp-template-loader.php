...
final class WFACP_Template_loader {
	...
	public function is_wfacp_checkout_page() {
		...
		if ( WFACP_Common::is_checkout_process() ) {
			$wfacp_post_id = filter_input( INPUT_POST, '_wfacp_post_id', FILTER_UNSAFE_RAW );
			$post          = get_post( $wfacp_post_id );
			WFACP_Common::pc( 'Try to get wfacp_id from posted data when checkout process initiated->' . $wfacp_post_id );
		}
		...
		if ( is_checkout() && ! ( is_order_received_page() || is_checkout_pay_page() ) ) {
			...
			if ( ! is_null( $may_be_post ) ) {
				...
				if ( ( WFACP_Common::get_post_type_slug() !== $may_be_post->post_type ) && apply_filters( 'wfacp_redirect_embed_global_checkout_url', true, $this->override_checkout_page_id, $may_be_post, $design_data ) ) {
				...
				if ( $may_be_post->post_status == 'publish' ) {
					if ( ! WFACP_Common::is_theme_builder() ) {
						$this->aero_post_data = $may_be_post;
						global $post;
						$post = $may_be_post;
	...
	public function localize_page_builder_texts() {
		...
		foreach ( $get_all_opted_page_builders as $builder ) {
			...
			$plugin_string   = sprintf( __( 'This template needs <strong>%s plugin</strong> activated.', 'woofunnels-aero-checkout' ), esc_html( $page_builder['title'] ) );
			$button_text     = __( 'Activate', 'woofunnels-aero-checkout' );
			$cancel_btn      = __( 'Cancel', 'woofunnels-aero-checkout' );
			$no_install      = 'no';
			$title           = __( 'Import Template', 'woofunnels-aero-checkout' );
			$show_cancel_btn = 'yes';
			...
			$string          = sprintf( __( ' Click the button to install and activate %s.', 'woofunnels-aero-checkout' ), esc_html( $page_builder['title'] ) );
			$install         = sprintf( __( ' Install and activate %s.', 'woofunnels-aero-checkout' ), esc_html( $page_builder['title'] ) );
			$builder_link    = '';
			/**
