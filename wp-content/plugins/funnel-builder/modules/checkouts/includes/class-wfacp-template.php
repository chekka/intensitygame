...
abstract class WFACP_Template_Common {
	...
	public function set_checkout_posted_data( $posted_data ) {
		...
		if ( ! empty( $this->address_keys ) ) {
			...
			if ( isset( $_REQUEST['billing_country'] ) && 'default' == trim( wc_clean( $_REQUEST['billing_country'] ) ) ) {
				$_REQUEST['billing_country'] = '';
			}
			if ( isset( $_REQUEST['shipping_country'] ) && 'default' == trim( wc_clean( $_REQUEST['shipping_country'] ) ) ) {
	...
	public function process_phone_field() {
		...
		if ( isset( $_POST['billing_phone'] ) && ! empty( $_POST['billing_phone'] ) && ! empty( $phone_data['billing']['number'] ) ) {
			if ( apply_filters( 'wfacp_intl_phone_code', false, $_POST ) ) {
				$code = '+' . $phone_data['billing']['code'];
			}

			$billing_phone             = $code . $phone_data['billing']['number'];
		...
		if ( isset( $_POST['shipping_phone'] ) && ! empty( $_POST['shipping_phone'] ) && ! empty( $phone_data['shipping']['number'] ) ) {
			$shipping_phone             = '+' . $phone_data['shipping']['code'] . $phone_data['shipping']['number'];
			if ( apply_filters( 'wfacp_intl_phone_code', false, $_POST ) ) {
				$code = '+' . $phone_data['shipping']['code'];
			}
			$shipping_phone             = $code . $phone_data['shipping']['number'];
			$_POST['shipping_phone']    = $shipping_phone;
			$_REQUEST['shipping_phone'] = $shipping_phone;
		}
	...
	public function update_custom_fields( $order_id, $posted_data ) {
		...
		foreach ( $advancedFields as $field_key => $field ) {
			...
			$field_value = wc_clean( $_REQUEST[ $field_key ] );// phpcs:ignore WordPress.Security.ValidatedSanitizedInput
