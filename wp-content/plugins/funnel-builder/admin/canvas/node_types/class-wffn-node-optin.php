...
class WFFN_Node_Optin extends WFFN_Base_Node_Step {
	...
	public function register_a_node( &$index, &$configs, &$edge_index ) {
		...
		$get_parent_config['tabs']  = array(
			array(
				...
				'api'   => class_exists( 'WFFN_Pro_Core' ) ? 'woofunnels-admin/canvas/' . $this->data['funnel_id'] . '/nodes/analytics/' . $this->id . '?type=optin' . $view_experiment : '',
			),
			array(
				'name'  => 'contacts',
				'title' => __( 'Contacts', 'funnel-builder' ),
				'api'   => class_exists( 'WFFN_Pro_Core' ) ? 'woofunnels-admin/canvas/' . $this->data['funnel_id'] . '/nodes/contacts/' . $this->id . '?type=optin' . $view_experiment : '',
			),
			array(
				'name'  => 'experiment',
				'title' => __( 'A/B Tests', 'funnel-builder' ),
				'api'   => class_exists( 'WFFN_Pro_Core' ) ? 'woofunnels-admin/experiment/?control=' . $this->id . '&data=true' : '',
			),
	...
	public function get_analytics_data( $step_id, $type, $start_date, $end_date, $is_interval, $int_request ) {
		...
		if ( isset( $this->data['view_experiment'] ) ) {
			$get_controller = BWFABT_Core()->controllers->get_integration( 'optin' );
		...
		$optin_sql  = "SELECT COUNT(id) as converted " . $interval_query . " FROM " . $wpdb->prefix . 'bwf_optin_entries' . " WHERE step_id IN(" . $step_ids . ") AND `" . $date_col . "` >= '" . $conv_date . "' AND `" . $date_col . "` < '" . $end_date . "' " . $group_by . " ORDER BY step_id ASC";
		$get_record = $wpdb->get_results( $optin_sql, ARRAY_A ); //phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		if ( method_exists( 'WFFN_Common', 'maybe_wpdb_error' ) ) {
			$db_error = WFFN_Common::maybe_wpdb_error( $wpdb );
			if ( true === $db_error['db_error'] ) {
				return $db_error;
		...
		$get_query = "SELECT SUM( CASE WHEN type = " . $view_type . " THEN `no_of_sessions` END ) AS viewed " . $interval_query . " FROM " . $wpdb->prefix . 'wfco_report_views' . "  WHERE object_id IN(" . $step_ids . ") AND `" . $date_col . "` >= '" . $start_date . "' AND `" . $date_col . "` < '" . $end_date . "' " . $group_by . " ORDER BY object_id ASC";
				...
				if ( is_array( $get_record ) && count( $get_record ) ) {
					...
					foreach ( $get_record as $record ) {
						if ( isset( $value['time_interval'] ) && isset( $record['time_interval'] ) && $value['time_interval'] === $record['time_interval'] ) {
							$value['converted'] = $record['converted'];
						}
						if ( 'interval' !== $is_interval ) {
							$value['converted'] = $record['converted'];
						}
