	...
	class WFFN_REST_Store_Checkout extends WP_REST_Controller {
		...
		public function get_store_checkout( WP_REST_Request $request ) {
			...
			$return = array(
				...
				'status'      => (int) WFFN_Core()->get_dB()->get_meta( $funnel_id, 'status' ),
				'steps'       => $steps,
				'count_data'  => array(
					'contacts' => WFFN_Core()->wffn_contacts->get_total_count( $funnel_id ),
					'steps'    => (false === $funnel->funnel_has_checkout())? $funnel->get_step_count() +1:$funnel->get_step_count() ,
				)
			);
		...
		public function import_funnels( WP_REST_Request $request ) {
			...
			if ( true === WFFN_Core()->import->validate_json( $funnels ) ) {
				...
				if ( isset( $request['override_existing'] ) && 'yes' === $request['override_existing'] && $request['id'] ) {
					$args       = [];
					$args['id'] = $request['id'];
				...
				if ( absint( $funnel_id ) > 0 ) {
					...
					$result = [
						'status'    => true,
						'funnel_id' => $funnel_id,
						'message'   => __( 'File successfully import.', 'funnel-builder' ),
					];
				}
			} else {
				$result = [
		...
		public function get_status( WP_REST_Request $request ) {
			...
			if ( $funnel_id > 0 ) {
				$status         = WFFN_Core()->get_dB()->get_meta( $funnel_id, 'status' );
				$resp['status'] = wffn_string_to_bool( $status );
			}
		...
		public function update_status( WP_REST_Request $request ) {
			...
			if ( $funnel_id > 0 ) {
				...
				$resp = array(
					...
					'message' => __( 'Status successfully updated', 'funnel-builder' ),
				);
			}
		...
		public function delete_store_step( $request ) {
			...
			if ( $funnel_id > 0 && ! empty( $type ) ) {
				...
				if ( $get_step instanceof WFFN_Step ) {
					...
					$deleted = $get_step->delete_step( $funnel_id, $step_id, false );
					if ( $deleted > 0 && 'wc_checkout' === $type ) {
				...
				if ( $get_substep instanceof WFFN_Substep ) {
					...
					if ( true === $native_checkout ) {
						...
						if ( $duplicate_id > 0 ) {
							...
							$resp['data'] = $get_substep->populate_substeps_data_properties( [ $data[ $type ][0]->id ] )[0];
