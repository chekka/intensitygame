...
if ( ! class_exists( 'WFFN_Admin' ) ) {
	class WFFN_Admin {
		...
		public function get_checkout_license_status() {
			$licence_exist = false;
			if ( wffn_is_wc_active() && method_exists( WFACP_Core()->importer, 'get_license_key' ) && false !== WFACP_Core()->importer->get_license_key() || ( false !== $this->get_license_status() ) ) {
				$licence_exist = true;
			}
			...
			if ( is_object( $License ) && is_array( $License->plugins_list ) && count( $License->plugins_list ) ) {
				foreach ( $License->plugins_list as $license ) {
					...
					$data = array(
						...
						'is_manually_deactivated' => isset( $license['_data']['manually_deactivated'] ) ? 1 : 0,
					);

					if ( $license['plugin'] === 'Funnel Builder Pro' ) {
