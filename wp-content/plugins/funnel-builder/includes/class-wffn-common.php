...
if ( ! class_exists( 'WFFN_Common' ) ) {
	class WFFN_Common {
		...
		public static function get_store_checkout_edit_link() {
			return add_query_arg( array(
				'page' => 'bwf',
				'path' => "/store-checkout",
			), admin_url( 'admin.php' ) );
		...
		public static function remove_orphaned_transients() {
			...
			$woofunnels_core_dir = $upload_dir . '/woofunnels/wffn-transient';
			$dir                 = @opendir( $woofunnels_core_dir . '/' ); // phpcs:ignore WordPress.CodeAnalysis.AssignmentInCondition.FoundInWhileCondition,Generic.PHP.NoSilencedErrors.Forbidden
			if ( false === $dir ) {
				return;
			}
			clearstatcache();
