	...
	class WFFN_WooFunnels_Support {
		...
		public function add_menus() {
			...
			if ( ! WooFunnels_dashboard::$is_core_menu && false !== $user ) {
				add_menu_page( __( 'WooFunnels', 'funnel-builder' ), __( 'WooFunnels', 'funnel-builder' ), $user, 'woofunnels', array(
					$this,
					'woofunnels_page',
				), '', 59 );
		...
		public function register_menu_for_pro() {
			...
			if ( false === wfacp_pro_dependency() ) {
				...
				add_submenu_page( 'woofunnels', null, '<a href="' . $link . '" style="background-color:#1DA867; color:white;" target="_blank"><strong>' . __( 'Upgrade to Pro', 'wp-marketing-automations' ) . '</strong></a>', 'manage_options', 'upgrade_pro', function () {
				}, 99 );
			}
