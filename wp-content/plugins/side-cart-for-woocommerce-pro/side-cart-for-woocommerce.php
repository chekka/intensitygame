<?php
/**
* Plugin Name: Side Cart For Woocommerce Pro
...
if (!defined('SCFW_BASE_NAME')) {
    define('SCFW_BASE_NAME', plugin_basename(SCFW_PLUGIN_FILE));
}
  	...
  	class SCFW {
    	...
    	public static function SCFW_instance() {
      	...
      	function init() {
	      	...
	      	add_filter( 'plugin_row_meta', array( $this, 'SCFW_plugin_row_meta' ), 10, 2 );
	    }

	    //Load all includes files
	    function includes() {
	    	include_once('includes/scfw_comman.php');
	    	include_once('includes/scfw_svg.php');
	    ...
	    function SCFW_load_script_style() {
			...
			wp_localize_script('SCFW-front_js', 'scfw_sidebar_width', array(
			    'scfw_width' => $scfw_comman[ 'wfc_sidecart_width'].'px',
			    'wfc_auto_open' => $scfw_comman['wfc_auto_open'],
			    'wfc_cart_open_from'=>$scfw_comman['wfc_cart_open_from'],
			    'wfc_trigger_class' => $scfw_comman['wfc_trigger_class'],
			));

	    }

		function SCFW_plugin_row_meta( $links, $file ) {
            if ( SCFW_BASE_NAME === $file ) {
                $row_meta = array(
                    'rating'    =>  '<a href="https://www.xeeshop.com/support-us/?utm_source=aj_plugin&utm_medium=plugin_support&utm_campaign=aj_support&utm_content=aj_wordpress" target="_blank">Support</a> |<a href="https://wordpress.org/support/plugin/side-cart-for-woocommerce/reviews/?filter=5" target="_blank"><img src="'.SCFW_PLUGIN_DIR.'/images/star.png" class="scfw_rating_div"></a>',
                );
