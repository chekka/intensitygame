...
class BWFAN_Abandoned_Cart {
	...
	public function remove_abandoned_data_from_table( $order_id ) {
		...
		} elseif ( true === apply_filters( 'bwfan_opt_cart_recovery_adv_logic', false, $billing_email, $order ) ) {
			/** Maybe attribute */
		...
		/** added code to remove tag and list when cart deleted */
		BWFAN_Common::bwfan_remove_abandoned_cart_tags( $cart_details, $order_id, $order );

		/** If automation contact exists, then try to delete the row */
		$contact = bwf_get_contact( 0, $cart_details['email'] );
		if ( 0 === absint( $contact->get_id() ) ) {
			return;
		}
		$result = BWFAN_Model_Automation_Contact::get_automation_contact_by_ab_id( $cart_details['ID'], $contact->get_id() );
		if ( empty( $result ) || ! isset( $result['ID'] ) ) {
			return;
		}
		if ( absint( $result['last'] ) > 0 ) {
			/** Mark automation complete */
			$data = [
				'cid'    => $result['cid'],
				'aid'    => $result['aid'],
				'event'  => $result['event'],
				's_date' => $result['c_date'],
		...
		BWFAN_Model_Automation_Contact::delete( $result['ID'] );
	}
	...
	public function bwfan_disable_abandoned_for_email( $should_execute, $email, $cart_details ) {
		...
		if ( empty( $email_status ) || ! isset( $email_status[ $email ] ) || empty( $email_status[ $email ] ) ) {
            ...
            AND   posts.post_status NOT IN ( '" . implode( "','", array_map( 'esc_sql', $failed_statuses ) ) . "' )
