...
class BWFAN_Delay_Controller extends BWFAN_Base_Step_Controller {
	...
	public function get_fixed_time() {
		...
		$diff = $current_store_timestamp - $current_timestamp;

		$fixed_date              = $this->data['date'];
		$fixed_date_timestamp    = strtotime( $fixed_date );
		$fixed_time              = $this->data['time_delay']['time'];
		$fixed_time_seconds      = BWFAN_Common::get_seconds_from_time_format( $fixed_time );
		$current_store_timestamp = intval( $fixed_date_timestamp ) + intval( $fixed_time_seconds );

		// if the diff value is in minus then add it else subtract it
		$current_store_timestamp = $diff < 0 ? $current_store_timestamp + $diff : $current_store_timestamp - $diff;

		if ( isset( $this->data['time_delay']['in_contact_timezone'] ) && ! empty( $this->data['time_delay']['in_contact_timezone'] ) ) {
			$this->is_contact_timezone = true;
		}

		/** Get time according to contact's timezone */
		$datetime = $this->get_contact_time( $current_store_timestamp, $fixed_time );
