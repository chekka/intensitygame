...
class BWFAN_Automation_Controller {
	...
	public function setup_automation() {
		...
		if ( false === $valid ) {
			$this->traverse_ins->maybe_traverse_back( $this );
		}
	...
	public function process_current_step() {
		...
		/**
		 * Get previous step id if skip step id is set
		 * For delay and jump step only
		 */
		$previous_step_id = 0;
		if ( ( self::$TYPE_WAIT === $this->type || self::$TYPE_JUMP === $this->type ) && ! empty( $this->skip_step_id ) ) {
			$previous_step_id = $this->traverse_ins->get_previous_step_id( $this->skip_step_id );
		}
		/** If previous step is start node */
		if ( 'start' === $previous_step_id ) {
			$this->step_id = 0;
		}

		/** Update last run step id and next step id */
		$this->last_step_id = intval( $previous_step_id ) > 0 ? $previous_step_id : $this->step_id;
