...
class BWFAN_Common {
	...
	public static function init() {
		...
		add_action( 'bwfan_lost_cart_triggered', [ __CLASS__, 'bwfan_lost_cart_triggered' ], 10, 1 );
	...
	public static function get_product_image( $product, $size = 'shop_catalog', $only_url = false, $img_width = '' ) {
		...
		/** checking for site url in image if not exists then pass image url in site url **/
		if ( ( strpos( $image_url, site_url() ) === false ) && ! filter_var( $image_url, FILTER_VALIDATE_URL ) ) {
	...
	public static function check_nonce() {
		...
		if ( wp_verify_nonce( $bwf_nonce, 'bwf_secure_key' ) || wp_verify_nonce( $nonce, 'bwfan-action-admin' ) || current_user_can( 'manage_options' ) ) {
			return;
		}
	...
	public static function as_pre_init_v2_cb() {
		...
		if ( ( ! isset( $_GET['rest_route'] ) || '/autonami/v2/worker' !== sanitize_text_field( $_GET['rest_route'] ) ) && false === strpos( $_SERVER['REQUEST_URI'], '/autonami/v2/worker' ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			return;
		}
	...
	public static function mark_abandoned_lost_cart() {
		...
		$query = "SELECT ID, email FROM {$wpdb->prefix}bwfan_abandonedcarts WHERE TIMESTAMPDIFF(MINUTE,last_modified,UTC_TIMESTAMP) > %d";
		$query = $wpdb->prepare( $query, $abandoned_time_in_days );

		$abandoned_ids = $wpdb->get_results( $query, ARRAY_A );

		if ( empty( $abandoned_ids ) ) {
			return;
		}

		$key  = 'bwf_lost_carts_' . time();
		$args = [ 'key' => $key ];
		if ( ! bwf_has_action_scheduled( 'bwfan_lost_cart_triggered', $args ) ) {
			bwf_schedule_recurring_action( time(), 120, 'bwfan_lost_cart_triggered', $args );
			update_option( $key, $abandoned_ids, false );
		}
	...
	public static function bwfan_lost_cart_triggered( $option_key ) {

		$carts = get_option( $option_key, [] );
		if ( empty( $carts ) ) {
			delete_option( $option_key );
			bwf_unschedule_actions( 'bwfan_lost_cart_triggered', [ 'key' => $option_key ] );
		...
		foreach ( $carts as $key => $cart ) {
			...
			/**Add lost tag and list to contact*/
			self::add_lost_cart_tags( $cart );

			$data = [ 'status' => 2 ];
			BWFAN_Model_Abandonedcarts::update( $data, array(
				'ID' => absint( $cart['ID'] ),
			) );
			// $wpdb->query( $wpdb->prepare( " UPDATE {$wpdb->prefix}bwfan_abandonedcarts SET `status` = %d WHERE ID = %d ", 2, $cart['ID'] ) );

			unset( $updated_carts[ $key ] );
			update_option( $option_key, $updated_carts, false );
		}
	}

	public static function add_lost_cart_tags( $cart ) {
		...
		$lost_cart_list = array_map( function ( $list ) {
		...
		$lost_cart_tags = array_map( function ( $tag ) {
		...
		if ( ! empty( $lost_cart_tags ) ) {
			$contact->set_tags( $lost_cart_tags );
		}

		if ( ! empty( $lost_cart_list ) ) {
			$contact->set_lists( $lost_cart_list );
		}
	...
	/** Search automation by title */
	public static function get_automation_by_title( $search, $version = 0, $ids = [] ) {
		global $wpdb;
		$query    = '';
		$id_query = '';
		...
		switch ( absint( $version ) ) {
			case  2 :
				$where = ! empty( $search ) ? " AND title LIKE '%$search%' " : '';
				$query = "SELECT ID as id, title  FROM {$wpdb->prefix}bwfan_automations WHERE 1=1 $where $id_query ORDER BY title ASC";
				break;
			case  1 :
				$where = ! empty( $search ) ? " AND am.meta_value LIKE '%$search%' AND am.meta_key = 'title'" : '';
		...
		$automations = ! empty( $query ) ? $wpdb->get_results( $query, ARRAY_A ) : [];

		/**search all automation from v1 and v2 */
		if ( 0 === absint( $version ) ) {
			$automations = self::search_automation_by_title( $search );
		}
	...
	/**
	 * search automation by title from automationmeta and automation table
	 */
	public static function search_automation_by_title( $search = '' ) {
		global $wpdb;
	...
	public static function get_setting_schema() {
		...
		$bounce_settings_schema = array_merge( $bounce_settings_schema, array(
			array(
				...
				'placeholder' => '15',
				'hint'        => __( 'Enter maximum email sending limit. Note: This is an indicative limit and not the actual sending rate for Autonami. The real sending rate would vary based on site configuration, plugin used and even SMTP API connection.', 'wp-marketing-automations' ),
				'required'    => false,
				'show'        => $show_fields,
				'toggler'     => array(),
			),
		...
		if ( bwfan_is_autonami_connector_active() && class_exists( 'BWFAN_Twilio_SMS_Unsubscribe' ) ) {
			...
			$twilio_sms_unsubscribe_settings = array(
				...
				'label'     => __( 'Twilio Unsubscribe Webhook URL', 'wp-marketing-automations' ),
				'type'      => 'copier',
				'class'     => 'bwfan_sms_webhook_twilio',
				'hint'      => __( "Paste this URL into your Twilio's settings to stop sending messages to the contacts. <a href='https://funnelkit.com/docs/autonami-2/how-to/receive-response-from-twilio/'>Learn more</a> on how to set up.", 'wp-marketing-automations' ),
				'required'  => false,
		...
		$sms_fields = array(
			...
			array(
				...
				'hint'          => __( 'Click tracking adds special variables to the URL that increases the SMS character limit. Enable this setting to reduce the character limit. Note: Autonami cannot track conversion rate if click tracking is disabled.', 'wp-marketing-automations' ),
					...
					array(
						...
						'fields'  => array(
							...
							array(
								...
								'hint'          => __( 'When enabled, logs will be saved to Autonami > Settings > Logs. Disable this settings after debugging is finished.', 'wp-marketing-automations' ),
		...
		foreach ( $bwf_licenses as $bwf_license ) {
			if ( $bwf_license['data_extra']['software_title'] === $plugin_name ) {
				if ( $onlyKey ) {
					...
					$bwf_license['data_extra']['manually_deactivated'] = isset( $bwf_license['manually_deactivated'] ) ? 1 : 0;
					if ( isset( $bwf_license['data_extra']['api_key'] ) ) {
						$bwf_license['data_extra']['api_key'] = 'xxxxxxxxxxxxxxxxxxxxxxxxxx' . substr( $bwf_license['data_extra']['api_key'], - 6 );
					}
	...
	public static function bwf_has_action_scheduled( $hook = '' ) {
		 ...
		 */
		if ( in_array( $hook, $list, true ) && is_array( self::$recurring_actions_db ) && isset( self::$recurring_actions_db[ $hook ] ) ) {
		...
		if ( in_array( $hook, $list, true ) ) {
			...
			$table = BWF_AS_Actions_Crud::_table();
			$p_key = BWF_AS_Actions_Crud::$primary_key;

			$placeholders = array_fill( 0, count( $list ), '%s' );
			$placeholders = implode( ', ', $placeholders );

			$query  = $wpdb->prepare( "SELECT `{$p_key}`, `hook` FROM $table WHERE `hook` IN ($placeholders)", $list );
	...
	public static function send_test_email( $args ) {
		...
		$type = isset( $args['type'] ) ? $args['type'] : 'rich';
		$data = [];
		...
		/** Email Pre-header */
		$pre_header = isset( $args['preheader'] ) ? $args['preheader'] : '';
		$body       = ! empty( $pre_header ) ? $send_email_ins->append_to_email_body( $body, $pre_header, '' ) : $body;

		/** Email Headers */
		$from_name      = isset( $args['senders_name'] ) ? $args['senders_name'] : '';
		$from_email     = isset( $args['senders_email'] ) ? $args['senders_email'] : '';
		$reply_to_email = isset( $args['reply_to'] ) ? $args['reply_to'] : '';

		if ( isset( $args['overRideSenderInfo'] ) && 1 === absint( $args['overRideSenderInfo'] ) ) {
			$from_name = isset( $global_email_settings['bwfan_email_from_name'] ) ? $global_email_settings['bwfan_email_from_name'] : $from_name;
			$from_name = isset( $args['overRideInfo']['from_name'] ) ? $args['overRideInfo']['from_name'] : $from_name;

			$from_email = isset( $global_email_settings['bwfan_email_from'] ) ? $global_email_settings['bwfan_email_from'] : $from_email;
			$from_email = isset( $args['overRideInfo']['from_email'] ) ? $args['overRideInfo']['from_email'] : $from_email;

			$reply_to_email = isset( $global_email_settings['bwfan_email_reply_to'] ) ? $global_email_settings['bwfan_email_reply_to'] : $reply_to_email;
			$reply_to_email = isset( $args['overRideInfo']['reply_to_email'] ) ? $args['overRideInfo']['reply_to_email'] : $reply_to_email;
		}
	...
	public static function bwfan_goal_step_updated( $step_id, $goal_run ) {
		...
		foreach ( $automations as $key => $automation_contact_id ) {
			...
			if ( ! empty( $status ) ) {
				...
				self::update_automation_contact_fields( $data['cid'], $data['aid'] );
			}
	...
	public static function bulk_contact_automation_end( $dynamic_str, $aid, $action, $status ) {
		...
		foreach ( $data as $key => $id ) {
			...
			self::update_automation_contact_fields( $automation_contact['cid'], $aid );
	...
	public static function perform_bulk_action( $a_cids, $option_key, $action = '' ) {
		...
		while ( ( time() - $start_time ) < 10 ) {
			...
			switch ( true ) {
				...
				case ( 'paused' === $action ) :
					$status = 3;
					self::update_automation_status( $ids, $status, $trails );
					break;
				case ( 'unpaused' === $action || 'retry' === $action ):
					$status = 1;
					self::update_automation_status( $ids, $status, $trails );
					break;
				case ( 'run_now' === $action ):
					$status = 1;
					$e_time = current_time( 'timestamp', 1 );
					self::update_automation_status( $ids, $status, $trails, $e_time );
					break;
			}
	...
	public static function update_automation_status( $ids, $status, $trails, $e_time = '' ) {
		BWFAN_Model_Automation_Contact::update_status_by_multiple_ids( $ids, $status, $e_time );
		if ( 1 === $status ) {
	...
	public static function delete_automation_v1_migrations( $aids ) {
		BWFAN_Core()->automations->delete_migrations( $aids );
