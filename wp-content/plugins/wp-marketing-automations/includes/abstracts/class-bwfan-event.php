...
abstract class BWFAN_Event {
	...
	public function run_v2( $automation, $event_slug ) {
		...
		$global_data = $event->get_event_data();
	...
	public function get_rule_group() {
		...
		$rule_groups = array_merge( $rule_groups, [ 'bwf_automation' ] );
	...
	public function update_automation_contact_field( $contact_id, $automation_id ) {
		...
		sort( $active_aids );
		$active_aids = wp_json_encode( array_unique( $active_aids ) );
		...
		sort( $entered_aids );
		$entered_aids = wp_json_encode( array_unique( $entered_aids ) );
