...
final class BWFAN_Wp_Sendemail extends BWFAN_Action {
	...
	public function get_final_email_header_settings( $step_data ) {
		...
		return array(
			'name'     => $override && ! empty( $email_data['from_name'] ) ? $email_data['from_name'] : $global_email_settings['bwfan_email_from_name'],
			'email'    => $override && is_email( $email_data['from_email'] ) ? $email_data['from_email'] : $global_email_settings['bwfan_email_from'],
			'reply_to' => $override && is_email( $email_data['reply_to_email'] ) ? $email_data['reply_to_email'] : $global_email_settings['bwfan_email_reply_to'],
		);
	}
	...
	public function email_content_v2( $data ) {
		...
		/** @var changes to replace the string in email body as string causing issue on saving  $body */
