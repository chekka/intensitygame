...
final class BWFAN_WC_New_Order extends BWFAN_Event {
	...
	public function new_order( $order_id ) {
		...
		if ( true === $automation_not_found ) {
			$goal_automations     = $this->get_current_goal_automations();
			$automation_not_found = empty( $goal_automations ) ? $automation_not_found : false;
		}

		if ( ( $automation_not_found || BWFAN_Common::bwf_check_to_skip_child_order( $order_id ) ) ) {
		...
		/** If goal found */
		if ( ! is_array( $goal_automations ) || 0 === count( $goal_automations ) ) {
			return;
		}
		update_post_meta( $order_id, '_bwfan_v2_' . $this->get_slug(), count( $goal_automations ) );
	}
	...
	public function order_status_changed( $order_id, $from_status, $to_status ) {
		...
		$v1_meta = BWFAN_Woocommerce_Compatibility::get_order_meta( $order_id, '_bwfan_' . $this->get_slug() );
		$v2_meta = BWFAN_Woocommerce_Compatibility::get_order_meta( $order_id, '_bwfan_v2_' . $this->get_slug() );
		if ( empty( $v1_meta ) && empty( $v2_meta ) ) {
			return;
		}
	...
	public function process( $order_id ) {
		...
		$data['automation_id'] = $this->event_automation_id;
	...
	public function validate_goal_settings( $automation_settings, $automation_data ) {
		...
		return in_array( $order_status, $status_selected );
	}
