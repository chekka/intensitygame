...
class BWFAN_Admin {
	...
	public function register_admin_menu() {
		...
		add_submenu_page( 'autonami', 'Automations', 'Automations <span style="background-color:#ece6e4; color: #000;white-space: nowrap; border-radius:10px;margin-left:2px;font-size:10px;padding:3px 6px;">Next Gen</span>', $capability, 'autonami&path=/automations', array(
			$this,
			'autonami_automations_page'
		), 24 );

		if ( BWFAN_Common::is_automation_v1_active() ) {
			add_submenu_page( 'autonami', 'Automations', 'Automations', $capability, 'autonami-automations', array( $this, 'autonami_automations_page' ), 25 );
		}
		...
		if ( false === bwfan_is_autonami_pro_active() ) {
			...
			add_submenu_page( 'autonami', null, '<a href="' . $link . '" style="background-color:#1DA867; color:white;" target="_blank"><strong>' . __( 'Upgrade to Pro', 'wp-marketing-automations' ) . '</strong></a>', $capability, null, function () {
			}, 60 );
		}
	}
	...
	public function schedule_abandoned_cart_cron() {
		...
		if ( ! $delete_expired_coupons || ! $mark_lost_carts ) {
			$date = new DateTime();
			$date->modify( '+1 days' );
		...
		if ( true === apply_filters( 'bwfan_ab_delete_inactive_carts', false ) ) {
			if ( ! BWFAN_Common::bwf_has_action_scheduled( 'bwfan_delete_old_abandoned_carts' ) ) {
				$date = new DateTime();
				$date->modify( '+1 days' );
				BWFAN_Common::convert_from_gmt( $date ); // convert to site time
				$date->setTime( 0, 0, 0 );
	...
	public function bwfan_add_order_contact_column_content( $column, $order_id ) {
		...
		/** @var using $default_currency_symbol to pass the default currency in case client using currency switcher as we show price in default currency */
		...
		if ( empty( $data ) || 0 === intval( $data['order'] ) ) {
			echo '-';
        ...
        <div style="display: flex; align-items: center">
            <span>
                <?php
                echo wc_price( $data['revenue'], array( 'currency' => $default_currency_symbol ) );
                echo ' | ';
