...
class BWFAN_Rule_Product_Stock extends BWFAN_Rule_Base {
	...
	public function is_match_v2( $automation_data, $rule_data ) {
		...
		$item    = isset( $automation_data['global']['single_item_id'] ) ? new WC_Order_Item_Product( $automation_data['global']['single_item_id'] ) : null;
		$order   = isset( $automation_data['global']['order_id'] ) ? wc_get_order( $automation_data['global']['order_id'] ) : null;
...
class BWFAN_Rule_Product_Item extends BWFAN_Rule_Products {
	...
	public function get_products( $automation_data = [] ) {
		...
		if ( ! empty( $automation_data ) && isset( $automation_data['global'] ) && is_array( $automation_data['global'] ) ) {
			...
			$order     = wc_get_order( $order_id );
			$cart_item = isset( $automation_data['global']['single_item_id'] ) && ! empty( $automation_data['global']['single_item_id'] ) ? new WC_Order_Item_Product( absint( $automation_data['global']['single_item_id'] ) ) : null;
		} else {
			$order     = BWFAN_Core()->rules->getRulesData( 'wc_order' );
			$cart_item = BWFAN_Core()->rules->getRulesData( 'wc_items' );
		}
...
class BWFAN_Rule_Order_Taxonomy extends BWFAN_Rule_Term_Taxonomy {
	...
	public function get_term_ids( $automation_data = [] ) {
		...
		if ( ! empty( $automation_data ) && isset( $automation_data['global'] ) && is_array( $automation_data['global'] ) ) {
			$cart_item = isset( $automation_data['global']['single_item_id'] ) && ! empty( $automation_data['global']['single_item_id'] ) ? new WC_Order_Item_Product( $automation_data['global']['single_item_id'] ) : 0;
			$order_id  = isset( $automation_data['global']['order_id'] ) ? $automation_data['global']['order_id'] : 0;
...
class BWFAN_Rule_Product_Tags extends BWFAN_Rule_Term_Taxonomy {
	...
	public function get_term_ids( $automation_data = [] ) {
		$all_terms = array();
		if ( ! empty( $automation_data ) && isset( $automation_data['global'] ) && is_array( $automation_data['global'] ) ) {
			$item_id  = isset( $automation_data['global']['single_item_id'] ) ? $automation_data['global']['single_item_id'] : 0;
			$order_id = isset( $automation_data['global']['order_id'] ) ? $automation_data['global']['order_id'] : 0;
...
class BWFAN_Rule_Product_Item_Type extends BWFAN_Rule_Base {
	...
	public function is_match_v2( $automation_data, $rule_data ) {
		 ...
		 *
		 *)
		 * Arrang data in index => value format
		 *
		 */
...
class BWFAN_Rule_Product_Item_Count extends BWFAN_Rule_Base {
	...
	public function is_match_v2( $automation_data, $rule_data ) {
		...
		$cart_item = isset( $automation_data['global']['single_item_id'] ) ? new WC_Order_Item_Product( $automation_data['global']['single_item_id'] ) : null;
