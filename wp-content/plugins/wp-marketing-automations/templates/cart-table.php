...
<div class='bwfan-email-cart-table bwfan-email-table-wrap'>
    <table cellspacing="0" cellpadding="6" border="1" width="100%">
        <thead>
        <tr>
            <th class="td" scope="col" <?php echo $colspan ?> style="<?php echo $text_align; ?> white-space: nowrap;"><?php esc_html_e( 'Product', 'woocommerce' ); ?></th>
            <th class="td" scope="col" style="width:90px;<?php echo $text_align; ?> white-space: nowrap;"><?php esc_html_e( 'Quantity', 'woocommerce' ); ?></th>
            <th class="td" scope="col" style="width:90px;<?php echo $text_align; ?> white-space: nowrap;"><?php esc_html_e( 'Price', 'woocommerce' ); ?></th>
        </tr>
        </thead>
        <tbody>
		...
		if ( false !== $cart ) {
			foreach ( $cart as $item ) :
                ...
                <tr>
                    ...
                    <td style="vertical-align:middle; <?php echo $text_align; ?> white-space: nowrap;">
						<?php
						if ( false === BWFAN_Merge_Tag_Loader::get_data( 'is_preview' ) ) {
			...
			foreach ( $products as $product ) {
                ...
                <tr>
					...
					?>
                    <td>
                        <h4 style="vertical-align:middle; <?php echo $text_align; ?> white-space: nowrap;"><?php echo wp_kses_post( 'Test Product' ); ?></h4>
                    </td>
                    <td style="vertical-align:middle; <?php echo $text_align; ?> white-space: nowrap;"
                    ">1</td>
            ...
            <tr>
                <th scope="row" <?php echo $colspan_foot ?> style=" <?php echo $text_align; ?>"><?php esc_html_e( 'Shipping', 'woocommerce' ); ?>
					<?php if ( wc_tax_enabled() && $tax_display !== 'excl' ):
						$tax_string = sprintf( '%s %s', BWFAN_Common::price( esc_attr( $data['shipping_tax_total'] ), $currency ), WC()->countries->tax_or_vat() );
						?>
                        <small><?php echo wp_kses_post( sprintf( __( '(includes %s)', 'woocommerce' ), $tax_string ) ) ?></small>
					<?php endif; ?>
                </th>
        ...
        <tr>
            <th scope="row" <?php echo $colspan_foot ?> style="<?php echo $text_align; ?>">
				...
				<?php if ( wc_tax_enabled() && $tax_display !== 'excl' ):
					$tax_string = sprintf( '%s %s', BWFAN_Common::price( $subtotal_tax, $currency ), WC()->countries->tax_or_vat() );
					?>
                    <small><?php echo wp_kses_post( sprintf( __( '(includes %s)', 'woocommerce' ), $tax_string ) ); ?></small>
				<?php endif; ?>
            </th>
