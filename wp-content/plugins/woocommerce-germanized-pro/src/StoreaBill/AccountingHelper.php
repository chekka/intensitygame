...
class AccountingHelper {
	...
	public static function improve_shortcode_format_support( $return_data, $format, $data, $atts ) {
		...
		if ( in_array( $atts['data'], $maybe_encrypted, true ) && ! empty( $return_data ) ) {
	...
	public static function check_accounting_enable() {
		if ( current_user_can( 'manage_woocommerce' ) && isset( $_GET['wc-gzdp-action'], $_GET['_wpnonce'] ) && 'activate-accounting' === $_GET['wc-gzdp-action'] && wp_verify_nonce( wp_unslash( $_GET['_wpnonce'] ), 'wc-gzdp-activate-accounting' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			update_option( 'woocommerce_gzdp_invoice_enable', 'yes' );

			wp_safe_redirect( esc_url_raw( remove_query_arg( array( 'wc-gzdp-action', '_wpnonce' ) ) ) );
		}
	}

	public static function accounting_disabled_notice() {
		if ( ! WC_germanized_pro()->enable_invoicing() && isset( $_GET['tab'] ) && ( $screen = get_current_screen() ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( 'woocommerce_page_wc-settings' === $screen->id && 'germanized-storeabill' === $_GET['tab'] && current_user_can( 'manage_woocommerce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$activation_link = wp_nonce_url( add_query_arg( array( 'wc-gzdp-action' => 'activate-accounting' ) ), 'wc-gzdp-activate-accounting' );

				echo '<div class="notice notice-error"><p>' . wp_kses_post( sprintf( __( 'The accounting feature is currently disabled. To edit PDF templates and settings, please <a href="%s">activate</a> the feature first.', 'woocommerce-germanized-pro' ), esc_url( $activation_link ) ) ) . '</p></div>';
			}
		}
	}
	...
	public static function maybe_round_additional_costs_tax_at_subtotal( $round_tax_at_subtotal, $order_item_type, $order ) {
		if ( in_array( $order_item_type, array( 'shipping', 'fee' ), true ) && self::enable_split_tax_calculation() ) {
	...
	public static function order_tax_is_moss( $is_oss, $tax, $order ) {
		if ( ! $is_oss && ( $tax_rate_id = $tax->get_rate_id() ) ) {
			...
			if ( $tax_rate && in_array( $tax_rate['tax_rate_class'], $virtual_rate_classes, true ) ) {
		...
		if ( $coupons = $order->get_items( 'coupon' ) ) {
			foreach ( $coupons as $coupon ) {
				if ( 'yes' === $coupon->get_meta( 'is_voucher', true ) ) {
					$total += (float) $coupon->get_discount();

					if ( $inc_tax ) {
						$total += (float) $coupon->get_discount_tax();
					}
				}
			}
		}
	...
	public static function small_business_shortcode( $atts ) {
		...
		if ( wc_gzd_is_small_business() ) {
			...
			$return = wp_strip_all_tags( ob_get_clean() );
		}
	...
	public static function register_item_preview_meta( $meta, $item, $preview ) {
		...
		$meta = array_merge(
			...
			array(
				...
				array(
					'title'   => __( 'Unit', 'woocommerce-germanized-pro' ),
					'preview' => wc_gzd_format_unit( _x( 'kg', 'unit', 'woocommerce-germanized-pro' ) ),
					'icon'    => '',
					'type'    => 'unit',
				),
				array(
		...
		if ( in_array( $preview->get_type(), array( 'invoice', 'invoice_cancellation' ), true ) ) {
			$meta = array_merge(
				$meta,
				array(
					array(
	...
	public static function add_phone() {
		...
		if ( ! empty( $phone ) ) {
			printf( esc_html_x( 'Phone: %s', 'storeabill-template', 'woocommerce-germanized-pro' ) . '<br>', esc_html( trim( $phone ) ) );
		}
	}
	...
	public static function add_vat_id() {
		...
		if ( ! empty( $vat_id ) ) {
			printf( esc_html_x( 'VAT ID: %s', 'storeabill-template', 'woocommerce-germanized-pro' ) . '<br>', esc_html( $vat_id ) );
		}
	}
}
