...
class DPD extends Auto {
	...
	protected function validate_label_request( $shipment, $args = array() ) {
		...
		if (
			( $shipment->is_shipping_domestic() && ! in_array( $args['product_id'], array_keys( Package::get_api()->get_domestic_products( $is_return ) ), true ) ) ||
			( $shipment->is_shipping_inner_eu() && ! in_array( $args['product_id'], array_keys( Package::get_api()->get_eu_products( $is_return ) ), true ) ) ||
			( $shipment->is_shipping_international() && ! in_array( $args['product_id'], array_keys( Package::get_api()->get_international_products( $is_return ) ), true ) )
		) {
