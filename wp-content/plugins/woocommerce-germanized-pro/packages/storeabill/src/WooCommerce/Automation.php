...
class Automation {
	...
	public static function has_invoice_timing( $timing ) {
		return self::get_invoice_timing() === $timing;
	}
	...
	protected static function sync_invoices_on_transactional_email() {
		add_filter(
			...
			function( $attachments, $email_id, $object, $email ) {
				$mails = self::get_invoice_emails();

				if ( ! in_array( $email_id, $mails, true ) ) {
					return $attachments;
				}
	...
	protected static function sync_invoices_on_order_status() {
		...
		$new_order_callback = function( $order ) use ( $statuses ) {
			...
			if ( $sab_order = Helper::get_order( $order ) ) {
				if ( in_array( $sab_order->get_status(), $statuses, true ) ) {
					$sync = true;
				}
			}
		...
		add_action(
			...
			function( $order_id ) use ( $new_order_callback ) {
				add_action(
					'woocommerce_after_order_object_save',
					function( $order ) use ( $order_id, $new_order_callback ) {
	...
	public static function sync_invoices( $order, $args = array() ) {
		...
		if ( self::invoice_gateway_specific() && ! empty( $payment_gateway ) ) {
			...
			if ( ! empty( $gateways ) && ! in_array( $payment_gateway, $gateways, true ) ) {
				return false;
			}
		}
