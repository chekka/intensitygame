...
class DefaultInvoiceCancellation extends Template {
	...
	public static function get_html() {
		...
		<p class="has-text-align-left">
			<strong><span style="font-size:25px" class="has-inline-text-size"><?php echo apply_filters( self::get_hook_prefix() . 'document_title', strtoupper( esc_html_x( 'Cancellation', 'storeabill-core', 'woocommerce-germanized-pro' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span></strong><br>
			<?php printf( esc_html_x( 'to Invoice %s', 'storeabill-core', 'woocommerce-germanized-pro' ), '<span class="document-shortcode sab-tooltip" contenteditable="false" data-tooltip="' . esc_attr_x( 'Parent invoice formatted number', 'storeabill-core', 'woocommerce-germanized-pro' ) . '" data-shortcode="document?data=parent_formatted_number"><span class="editor-placeholder"></span>' . wp_kses_post( $preview->get_parent_formatted_number() ) . '</span>' ); ?>
		</p>
		<!-- /wp:storeabill/document-title -->
