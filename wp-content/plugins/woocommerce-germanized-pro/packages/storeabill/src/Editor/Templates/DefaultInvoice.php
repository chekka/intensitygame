...
class DefaultInvoice extends Template {
	...
	public static function get_html() {
		...
		<div class="wp-block-storeabill-address sab-document-address has-text-align-left">
			<p class="address-heading">
				<span style="font-size:10px" class="has-inline-text-size"><span style="color:<?php echo esc_attr( self::get_light_color() ); ?>" class="has-inline-color"><?php echo apply_filters( self::get_hook_prefix() . 'address_header', wp_kses_post( Countries::get_formatted_base_address( ' - ' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span></span>
			</p>
		...
		<p class="has-text-align-left" style="font-size:25px"><strong><?php echo apply_filters( self::get_hook_prefix() . 'document_title', esc_html( strtoupper( _x( 'Invoice', 'storeabill-core', 'woocommerce-germanized-pro' ) ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong></p>
		...
		<div class="wp-block-columns">
			...
			<div class="wp-block-column">
				<!-- wp:paragraph -->
				<p>
					<?php echo esc_html_x( 'Invoice number', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>: <span class="document-shortcode sab-tooltip" contenteditable="false" data-tooltip="<?php echo esc_attr_x( 'Formatted document number', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>" data-shortcode="document?data=formatted_number"><span class="editor-placeholder"></span><?php echo wp_kses_post( $preview->get_formatted_number() ); ?></span><br>
					<?php echo esc_html_x( 'Order number', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>: <span class="document-shortcode sab-tooltip" contenteditable="false" data-tooltip="<?php echo esc_attr_x( 'Order number', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>" data-shortcode="document?data=order_number"><span class="editor-placeholder"></span><?php echo wp_kses_post( $preview->get_order_number() ); ?></span><br>
					<?php echo esc_html_x( 'Payment method', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>: <span class="document-shortcode sab-tooltip" contenteditable="false" data-tooltip="<?php echo esc_attr_x( 'Payment method', 'storeabill-core', 'woocommerce-germanized-pro' ); ?>" data-shortcode="document?data=payment_method_title"><span class="editor-placeholder"></span><?php echo esc_html( $preview->get_payment_method_title() ); ?></span><br>
		...
		<div class="wp-block-storeabill-item-table is-style-even has-border-horizontal">
			...
			<div class="wp-block-storeabill-item-table-column is-horizontally-aligned-left" style="flex-basis:45%">
				...
				<?php if ( self::show_differential_taxation_notice() ) : ?>
					...
					<p class="wp-block-storeabill-item-differential-taxation-notice sab-block-item-content has-text-color has-small-font-size" style="color:<?php echo esc_attr( self::get_light_color() ); ?>"><?php echo apply_filters( self::get_hook_prefix() . 'differential_taxation_notice', esc_html_x( 'Subject to differential taxation under §25a UStG.', 'storeabill-core', 'woocommerce-germanized-pro' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
		...
		<!-- wp:storeabill/reverse-charge-notice -->
		<p class="has-text-align-left"><?php echo apply_filters( self::get_hook_prefix() . 'reverse_charge_notice', esc_html_x( 'Tax liability of the recipient of the services.', 'storeabill-core', 'woocommerce-germanized-pro' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
		<!-- /wp:storeabill/reverse-charge-notice -->

		<!-- wp:storeabill/third-country-notice -->
		<p class="has-text-align-left"><?php echo apply_filters( self::get_hook_prefix() . 'third_country_notice', esc_html_x( 'Tax-exempt export delivery.', 'storeabill-core', 'woocommerce-germanized-pro' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
		<!-- /wp:storeabill/third-country-notice -->
