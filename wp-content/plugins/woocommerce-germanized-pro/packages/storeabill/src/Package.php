...
class Package {
	...
	public static function upload_dir_warning() {
		...
		<div class="notice notice-error error">
			<p><?php printf( esc_html_x( 'Your document upload directory is missing. Please manually create the folder %s and make sure that it is writeable.', 'storeabill-core', 'woocommerce-germanized-pro' ), '<i>wp-content/uploads/' . esc_html( $dirname ) . '</i>' ); ?></p>
		</div>
		<?php
	}
	...
	public static function register_document_types() {
		sab_register_document_type(
			...
			array(
				...
				'statuses'               => array_replace_recursive(
					sab_get_document_statuses(),
					array(
	...
	public static function setup_document( $document ) {
		 ...
		 */
		if ( $template = $document->get_template() ) {
			$GLOBALS['post'] = get_post( $template->get_id() ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		}
	...
	public static function register_data_stores( $stores ) {
		$data_stores = apply_filters(
			'storeabill_data_stores',
			array(
