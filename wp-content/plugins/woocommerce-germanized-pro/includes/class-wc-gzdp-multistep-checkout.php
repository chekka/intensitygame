...
class WC_GZDP_Multistep_Checkout {
	...
	public function ajax_refresh_step() {
		...
		if ( ! isset( $_POST['wc_gzdp_multistep_refresh_step'] ) || ! wp_verify_nonce( wp_unslash( $_POST['wc_gzdp_multistep_refresh_step'] ), 'wc_gzdp_multistep_refresh_step' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			wp_send_json_error( 'bad_nonce' );
			wp_die();
		}

		$step_id = isset( $_POST['step'] ) ? wc_clean( wp_unslash( $_POST['step'] ) ) : 'address';
		$exists  = false;
	...
	public function get_custom_css() {
		...
		if ( 'navigation' === $layout_style ) {
			$colors = apply_filters(
				'woocommerce_gzdp_checkout_step_colors',
				array(
	...
	public function set_scripts( $assets ) {
		...
		if ( apply_filters( 'woocommerce_gzdp_multistep_checkout_enable_payment_compatibility_mode', $enable_payment_compatibility_script ) ) {
			...
			wp_localize_script(
				...
				'wc_gzdp_multistep_checkout_payment_compatibility_params',
				array(
					'gateways'          => apply_filters(
						'woocommerce_gzdp_multistep_checkout_payment_compatibility_gateways',
						array(
					...
					'extended_gateways' => apply_filters(
						'woocommerce_gzdp_multistep_checkout_payment_compatibility_extended_gateways',
						array(
							'woocommerce_payments',
						)
					),
	...
	public function check_step_submit() {
		...
		if ( isset( $_POST['wc_gzdp_step_submit'] ) && ! empty( $_POST['wc_gzdp_step_submit'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$action = ( isset( $_POST['wc_gzdp_step_refresh'] ) ? 'refresh' : 'submit' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing

			if ( $step = $this->get_step( wc_clean( wp_unslash( $_POST['wc_gzdp_step_submit'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function get_step_names() {
		return apply_filters(
			'woocommerce_gzdp_checkout_steps',
			array(
	...
	public function get_address_data( $type ) {
		...
		$fields = apply_filters(
			'woocommerce_gzdp_checkout_formatted_' . $type . '_address_fields',
			array(
				'first_name',
				'last_name',
				'company',
