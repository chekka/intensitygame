...
class WC_GZDP_VAT_Helper {
	...
	public function vat_result_meta_box() {
		...
		if ( $order = wc_get_order( $post ) ) {
			...
			?>
			<ul class="vat-data">
				<li><strong><?php esc_html_e( 'VAT ID', 'woocommerce-germanized-pro' ); ?></strong> <?php echo esc_attr( $this->get_order_vat_id( $order ) ); ?></li>
	...
	public function get_vat_id_from_string( $number, $expected_country = '' ) {
		 ...
		 */
		if ( ! empty( $expected_country ) && WC()->countries->get_base_country() !== $expected_country && apply_filters( 'woocommerce_gzdp_skip_vat_id_country_validation', true, $expected_country ) ) {
			$expected_country = '';
		}
	...
	public function vat_field_is_required( $for_registration = false ) {
		...
		if ( $this->country_supports_vat_id( $location[0], $location[2] ) ) {
			...
			if ( ! $is_required && 'yes' === get_option( 'woocommerce_gzdp_force_virtual_product_business' ) && $this->country_supports_vat_exempt( $location[0], $location[2] ) ) {
	...
	public function validate( $country, $number, $address_expected = array() ) {
		...
		} elseif ( in_array( $country, wc_gzdp_get_eu_vat_countries(), true ) || 'XI' === $country ) {
		...
		if ( apply_filters( 'woocommerce_gzdp_vat_validator', $vat_validator, $country ) ) {
			/**
			...
			if ( apply_filters( 'woocommerce_gzdp_vat_validation_result', $vat_validator->check( $country, $number ), $country, $number ) ) {
				...
				// Save the result as transient (1 day min)
				set_transient( 'vat_id_validation_result_' . $transient_postfix, $vat_result, ( max( 1, $days ) * DAY_IN_SECONDS ) );
