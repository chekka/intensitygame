	...
	class WC_GZDP_Install {
		...
		public static function setup_redirect() {
			if ( get_option( '_wc_gzdp_setup_wizard_redirect' ) ) {
				...
				if ( ( isset( $_REQUEST['action'] ) && 'upgrade-plugin' === $_REQUEST['action'] ) && ( isset( $_REQUEST['plugin'] ) && strstr( wp_unslash( $_REQUEST['plugin'] ), 'woocommerce-germanized-pro.php' ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					return;
				}

				delete_option( '_wc_gzdp_setup_wizard_redirect' );
				wp_safe_redirect( esc_url_raw( admin_url( 'admin.php?page=wc-gzdp-setup' ) ) );
				exit();
			}
		}
		...
		public static function install() {
			if ( ! function_exists( 'WC' ) ) {
				wp_die( wp_kses_post( sprintf( __( 'Please install and activate <a href="%s" target="_blank">WooCommerce</a> before installing Germanized Pro. Thank you!', 'woocommerce-germanized-pro' ), 'http://wordpress.org/plugins/woocommerce/' ) ) );
			}
			...
			update_option( 'woocommerce_gzdp_activation_date', date( 'Y-m-d' ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
		...
		protected static function create_tables() {
			...
			$result  = false;
			$columns = $wpdb->get_results( "DESC {$wpdb->term_taxonomy}" );

			foreach ( $columns as $row ) {
				if ( 'order' === $row->Field ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					$result = true;
					break;
				}
			}
			...
			if ( ! $result ) {
				update_option( 'woocommerce_gzdp_term_taxonomy_misses_order_column', 'yes' );
			}
		}
		...
		protected static function get_memory_limit() {
			...
			switch ( substr( $memory_limit, -1 ) ) {
				case 'M':
				case 'm':
					return intval( $memory_limit ) * 1048576;
				case 'K':
				case 'k':
					return intval( $memory_limit ) * 1024;
				case 'G':
				case 'g':
					return intval( $memory_limit ) * 1073741824;
				default:
					return intval( $memory_limit ) * 1048576;
			}
		}
		...
		public static function legacy_invoice_import_notice() {
			...
			<div id="message" class="notice">
				<p>
					<?php echo wp_kses_post( sprintf( __( 'Germanized Pro legacy invoice import is running. Check the <a href="%1$s">logs</a> for further information. You can <a href="%2$s" onclick="%3$s">cancel</a> the import at any time.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=wc-status&tab=logs' ) ), esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=wc_gzdp_legacy_invoice_import_cancel' ), 'wc-gzdp-legacy-invoice-import-cancel' ) ), 'return confirm(\'' . __( 'Do you really want to cancel the import?', 'woocommerce-germanized-pro' ) . '\')' ) ); ?>

					<?php if ( get_option( '_wc_gzdp_show_invoice_automation_upgrade_disabled_notice' ) ) : ?>
						<?php echo wp_kses_post( sprintf( __( 'We\'ve temporarily disabled automatic invoice creation to prevent duplicates. After the import is finished (you\'ll be notified) you might want to <a href="%s">re-enable the setting</a>.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=germanized-storeabill' ) ) ) ); ?>
					<?php endif; ?>
				</p>
				<?php if ( ! $has_next ) : ?>
					<p>
						<?php echo wp_kses_post( sprintf( __( 'Seems like the importer has failed or stop. You\'ll might need to <a href="%s">restart</a> the importer.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=wc-status&tab=germanized' ) ) ) ); ?>
					</p>
				<?php endif; ?>
				<?php if ( ! empty( $errors ) ) : ?>
			...
			<div id="message" class="notice updated">
				<h3><?php esc_html_e( 'Germanized Pro document import ready', 'woocommerce-germanized-pro' ); ?></h3>
				<p>
					<?php echo wp_kses_post( sprintf( __( 'Your documents have been imported successfully. Check out the new <a href="%s">accounting page</a>.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=sab-accounting' ) ) ) ); ?>
				</p>
				<?php if ( get_option( '_wc_gzdp_show_invoice_automation_upgrade_disabled_notice' ) ) : ?>
					<p>
						<?php echo wp_kses_post( sprintf( __( 'During import we\'ve disabled automatic invoice creation to prevent duplicates. After you\'ve reviewed your document templates you might want to <a href="%s">re-enable the setting</a>.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=germanized-storeabill' ) ) ) ); ?>
					</p>
				<?php endif; ?>
				<?php if ( ! empty( $errors ) ) : ?>
					<?php include WC_GERMANIZED_PRO_ABSPATH . 'includes/admin/views/html-import-legacy-invoices-errors.php'; ?>
				...
				<p class="button-wrapper">
					<a class="button button-secondary" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=wc_gzdp_legacy_invoice_import_cancel' ), 'wc-gzdp-legacy-invoice-import-cancel' ) ); ?>"><?php esc_html_e( 'Remove notice', 'woocommerce-germanized-pro' ); ?></a>
				</p>
			</div>
				<?php
			}
		...
		public static function legacy_invoice_import_cancel_submit() {
			...
			if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( wp_unslash( $_GET['_wpnonce'] ), 'wc-gzdp-legacy-invoice-import-cancel' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				wp_die();
			}
			...
			if ( wp_get_referer() ) {
				...
				wp_safe_redirect( esc_url_raw( admin_url( 'admin.php?page=sab-accounting' ) ) );
			}
		}
		...
		public static function legacy_invoice_import_submit() {
			...
			if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['_wpnonce'] ), 'wc-gzdp-legacy-invoice-import-start' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				wp_die();
			}
		...
		public static function legacy_invoice_import( $limit, $after ) {
			...
			if ( empty( $invoices ) ) {
				...
				if ( $current_ids == $last_post_ids ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
					/**
		...
		public static function update() {
			...
			if ( version_compare( $current_db_version, '1.2.0', '<' ) ) {
				...
				self::upgrade_3_4_2();
			} elseif ( version_compare( $current_db_version, '3.5.2', '<' ) ) {
				self::upgrade_3_5_2();
		...
		private static function upgrade_3_5_4() {
			if ( ! taxonomy_exists( 'product_nutrient' ) ) {
				return;
			}

			/**
			 * Maybe insert missing units
			 */
			wp_insert_term( 'kcal', 'product_unit', array( 'slug' => 'kcal' ) );
			wp_insert_term( 'kJ', 'product_unit', array( 'slug' => 'kj' ) );
			...
			foreach ( $nutrients as $nutrient_data ) {
				$slug = empty( $nutrient_data['slug'] ) ? sanitize_title( $nutrient_data['title'] ) : sanitize_title( $nutrient_data['slug'] );

				if ( $nutrient = \Vendidero\Germanized\Pro\Food\Helper::get_nutrient( $slug ) ) {
					if ( $unit = WC_germanized()->units->get_unit_term( $nutrient_data['unit'] ) ) {
		...
		private static function maybe_update_nutrient( $nutrient_data ) {
			...
			if ( $nutrient = \Vendidero\Germanized\Pro\Food\Helper::get_nutrient( $slug ) ) {
				$nutrient->set_is_mandatory( $nutrient_data['is_mandatory'] );
		...
		protected static function upgrade_3_5_2() {
			if ( ! taxonomy_exists( 'product_nutrient' ) ) {
				return;
			}

			foreach ( \Vendidero\Germanized\Pro\Food\Helper::get_default_nutrients() as $nutrient_data ) {
				$nutrient_data = wp_parse_args(
					$nutrient_data,
					array(
						'children' => array(),
				...
				$updated = self::maybe_update_nutrient( $nutrient_data );

				if ( $updated ) {
					foreach ( $nutrient_data['children'] as $child_data ) {
						self::maybe_update_nutrient( $child_data );
					}
				}
			}
		}
		...
		protected static function upgrade_3_0_0() {
			...
			if ( 'yes' === get_option( 'woocommerce_gzdp_invoice_enable' ) && get_option( 'wc_gzdp_invoice_simple' ) ) {
				...
				if ( 'yes' === get_option( 'woocommerce_gzdp_invoice_auto' ) ) {
					...
					if ( ! empty( $gateways_enabled ) ) {
						...
						foreach ( \Vendidero\StoreaBill\WooCommerce\Helper::get_available_payment_methods() as $method ) {
							...
							if ( in_array( $method_id, $gateways_enabled, true ) ) {
				...
				if ( ! empty( $frontend_types ) && is_array( $frontend_types ) ) {
					if ( in_array( 'simple', $frontend_types, true ) ) {
						update_option( 'storeabill_invoice_woo_order_invoice_customer_download', 'yes' );
					}

					if ( in_array( 'cancellation', $frontend_types, true ) ) {
		...
		public static function create_legacy_template( $document_type = 'invoice', $make_default = true ) {
			...
			if ( $is_invoice ) {
				...
				if ( in_array( get_option( 'woocommerce_gzdp_invoice_net_totals' ), array( 'always', 'greater_250' ), true ) ) {
			...
			$before_table_callback = function() use ( $before_table ) {
				?>
			<!-- wp:paragraph -->
				<?php echo wp_kses_post( self::convert_legacy_shortcodes( $before_table ) ); ?>
			<!-- /wp:paragraph -->
				<?php
			};
			...
			$after_table_callback = function() use ( $after_table ) {
				?>
			<!-- wp:paragraph -->
				<?php echo wp_kses_post( self::convert_legacy_shortcodes( $after_table ) ); ?>
			<!-- /wp:paragraph -->
				<?php
			};
			...
			if ( $margins_first_page != $margins ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
		...
		protected static function strip_tags( $html ) {
			return strip_tags( $html, '<br><strong><em>' );
		}
		...
		protected static function get_legacy_template( $template ) {
			...
			if ( ! empty( $template ) ) {
				...
				if ( @file_exists( $file ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
					return $template;
				}
			}
		...
		protected static function create_template( $document_type, $args = array() ) {
			...
			<!-- wp:storeabill/document-styles /-->
				<?php if ( empty( $args['first_page_template_id'] ) ) : ?>
					<?php echo $editor_tpl::get_default_header(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php endif; ?>
				<?php if ( empty( $args['first_page_template_id'] ) ) : ?>
					<?php echo $editor_tpl::get_default_footer(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php endif; ?>
				<?php
		...
		protected static function get_legacy_margins( $margins ) {
			if ( empty( $margins ) || ! is_array( $margins ) || count( $margins ) < 4 ) {
		...
		public static function upgrade_fonts_path_suffix() {
			...
			if ( file_exists( $path ) && file_exists( $new_gzdp_dir ) ) {
				...
				$files = @glob( $path . '/*.*' ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged

				foreach ( $files as $file ) {
					$file_to_go = str_replace( $path, $new_gzdp_dir, $file );
					@rename( $file, $file_to_go ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
				}
			}
		}
				...
				if ( $invoice_settings && is_array( $invoice_settings ) ) {
					...
					foreach ( $types as $type ) {
						if ( isset( $invoice_settings[ $type ] ) ) {
							...
							if ( 'invoice_cancellation' === $invoice_type ) {
