...
class WC_GZDP_Settings_Tab_Taxes extends WC_GZD_Settings_Tab_Taxes {
	...
	protected function get_vat_id_settings() {
		return array(
			array(
				'title' => __( 'VAT ID', 'woocommerce-germanized-pro' ),
				'type'  => 'title',
				'id'    => 'vat_id_options',
			),
			...
			array(
				...
				'desc'              => __( 'Check and store actual customer location (based on IP address) while selling virtual products.', 'woocommerce-germanized-pro' ) . '<div class="wc-gzd-additional-desc">' . sprintf( __( 'Store customer location data based on <a href="%1$s">Geolocation</a>. Shows a <a href="%2$s">checkbox</a> in case the IP-based country differs from the country chosen by the customer.', 'woocommerce-germanized-pro' ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=integration' ) ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=germanized-checkboxes&checkbox_id=virtual_vat_location' ) ) ) . '</div>',
	...
	protected function after_save( $settings, $current_section = '' ) {
		if ( '' === $current_section ) {
			...
			if ( isset( $_POST['woocommerce_gzdp_enable_vat_check'] ) && ! empty( $_POST['woocommerce_gzdp_enable_vat_check'] ) && ! class_exists( 'SoapClient' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			if ( isset( $_POST['woocommerce_gzdp_vat_requester_vat_id'] ) && ! empty( $_POST['woocommerce_gzdp_vat_requester_vat_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				if ( ! class_exists( 'WC_GZDP_VAT_Helper' ) ) {
					WC_germanized_pro()->load_vat_module();
				}

				$vat_id = WC_GZDP_VAT_Helper::instance()->get_vat_id_from_string( wc_clean( wp_unslash( $_POST['woocommerce_gzdp_vat_requester_vat_id'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
				update_option( 'woocommerce_gzdp_vat_requester_vat_id', $vat_id['country'] . $vat_id['number'] );
			}

			// Force virtual VAT checkbox to be enabled in case the option is active
			if ( isset( $_POST['woocommerce_gzdp_virtual_vat_location_check'] ) && ! empty( $_POST['woocommerce_gzdp_virtual_vat_location_check'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				if ( class_exists( 'WC_GZD_Legal_Checkbox_Manager' ) ) {
					$manager = WC_GZD_Legal_Checkbox_Manager::instance();

					if ( $checkbox = $manager->get_checkbox( 'virtual_vat_location' ) ) {
						$checkbox->update_option( 'is_enabled', 'yes' );
					}
				}
			}

			// Disable the checkbox on disabling the VAT check of check.
			if ( ! isset( $_POST['woocommerce_gzdp_virtual_vat_location_check'] ) || ! isset( $_POST['woocommerce_gzdp_enable_vat_check'] ) ) {
				if ( class_exists( 'WC_GZD_Legal_Checkbox_Manager' ) ) {
					$manager = WC_GZD_Legal_Checkbox_Manager::instance();
