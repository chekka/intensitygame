...
class WC_GZDP_Admin {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_gzd_status_after_germanized', array( $this, 'version_output' ) );
	...
	public function create_default_food_terms() {
		...
		?>
		<tr>
			<td><?php esc_html_e( 'Nutrients & Allergenic', 'woocommerce-germanized-pro' ); ?></td>
			...
			<td>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'create-food-terms' => true ) ), 'wc-gzdp-create-food-terms' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Create missing default terms', 'woocommerce-germanized-pro' ); ?></a></td>
			</td>
		</tr>
		<?php
	}
	...
	public function generator_cache_clean() {
		?>
		<tr>
			<td><?php esc_html_e( 'Clear generator cache', 'woocommerce-germanized-pro' ); ?></td>
			...
			<td>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'clear-generator-cache' => true ) ), 'wc-gzdp-clear-generator-cache' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Clear cache', 'woocommerce-germanized-pro' ); ?></a></td>
			</td>
		</tr>
		<?php
	}
	...
	public function legacy_invoice_import() {
		...
		?>
		<tr>
			<td><?php esc_html_e( 'Import legacy documents', 'woocommerce-germanized-pro' ); ?></td>
			<td class="help"></td>
			<td>
				<p>
	...
	public function version_output() {
		?>
		<tr>
			<td data-export-label="WC GZD Pro Version"><?php esc_html_e( 'Pro Version', 'woocommerce-germanized-pro' ); ?>:</td>
			<td class="help">&nbsp;</td>
			<td><?php echo esc_html( WC_germanized_pro()->version ); ?></td>
		</tr>
		<tr>
			<td data-export-label="WC GZD Pro Database Version"><?php esc_html_e( 'Pro Database Version', 'woocommerce-germanized-pro' ); ?>:</td>
			<td class="help">&nbsp;</td>
			<td><?php echo esc_html( get_option( 'woocommerce_gzdp_db_version' ) ); ?></td>
		</tr>
		<?php
	}
	...
	public function legacy_template_import() {
		...
		?>
		<tr>
			<td><?php esc_html_e( 'Import legacy templates', 'woocommerce-germanized-pro' ); ?></td>
			<td class="help"></td>
			<td>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'import-legacy-templates' => true ) ), 'wc-gzdp-import-legacy-templates' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Import legacy templates', 'woocommerce-germanized-pro' ); ?></a></td>
			</td>
		</tr>
		<?php
	}
	...
	public function check_import_legacy_templates() {
		if ( current_user_can( 'manage_woocommerce' ) && isset( $_GET['import-legacy-templates'] ) && isset( $_GET['_wpnonce'] ) && check_admin_referer( 'wc-gzdp-import-legacy-templates' ) ) {
			...
			wp_safe_redirect( esc_url_raw( admin_url( 'admin.php?page=wc-settings&tab=germanized-storeabill' ) ) );
		}
	}
	...
	public function get_custom_setting_types() {
		return array( 'decimal', 'disclaimer', 'editor', 'notice', 'select_page', 'checkbox_multiple', 'attachment' );
	}
	...
	public function is_custom_setting_type( $type ) {
		...
		return in_array( $type, $this->get_custom_setting_types(), true );
	}
	...
	public function sanitize_attachment_field( $value ) {
		$option_value = ( isset( $_POST[ $value['id'] ] ) ? explode( ',', stripslashes_deep( $_POST[ $value['id'] ] ) ) : array() ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function save_multiples( $settings ) {
		foreach ( $settings as $value ) {
			...
			if ( $this->is_custom_setting_type( $value['type'] ) && in_array( $type, array( 'attachment', 'checkbox_multiple' ), true ) && method_exists( $this, 'sanitize_' . $type . '_field' ) ) {
		...
		<tr valign="top">
			...
			<td class="forminp forminp-attachment wc-gzdp-attachment-wrapper">
				...
				<input 
					...
					type="button" 
					value="<?php echo esc_html_x( 'Select or upload attachment', 'admin-field', 'woocommerce-germanized-pro' ); ?>"
				/>
				<div class="current-attachment">
					<?php if ( ! empty( $option_value ) ) : ?>
						<?php foreach ( (array) $option_value as $option ) : ?>
							<div class="wc-gzdp-unset-attachment-wrap data-unset-<?php echo esc_attr( $option ); ?>">
								<a class="unset-attachment" href="#" data-unset="<?php echo esc_attr( $option ); ?>"><span class="dashicons dashicons-no-alt"></span></a>
								<a class="" href="<?php echo esc_url( get_edit_post_link( $option ) ); ?>" target="_blank"><?php echo esc_html( get_the_title( $option ) ); ?></a>
							</div>
						<?php endforeach; ?>
					<?php endif; ?>
				</div>
	...
	public function sanitize_decimal_field( $value ) {
		$option_value = isset( $_POST[ $value['id'] ] ) ? wc_format_decimal( stripslashes_deep( $_POST[ $value['id'] ] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		<tr valign="top">
			...
			<td class="forminp forminp-gzdp-decimal wc-gzdp-decimal-wrapper">
				<input 
					...
					type="text"
					style="<?php echo esc_attr( $value['css'] ); ?>"
					value="<?php echo esc_attr( wc_format_localized_decimal( $option_value ) ); ?>"
	...
	public function sanitize_editor_field( $value ) {
		$option_value = ( isset( $_POST[ $value['id'] ] ) ? trim( wp_filter_post_kses( wp_unslash( $_POST[ $value['id'] ] ) ) ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function editor_field( $value ) {
		...
		<tr valign="top">
			...
			<td class="forminp forminp-editor wc-gzdp-editor-wrapper">
				<?php
				wp_editor(
					htmlspecialchars_decode( $option_value ),
					$value['id'],
					array(
						'textarea_name' => $value['id'],
						'textarea_rows' => 5,
						'editor_class'  => $value['class'],
						'editor_css'    => '<style>' . $value['css'] . '</style>',
						'media_buttons' => false,
						'teeny'         => true,
					)
				);
				...
				<input type="hidden" name="<?php echo esc_attr( $value['id'] ); ?>-attribute-container" id="<?php echo esc_attr( $value['id'] ); ?>-attribute-container" value="" <?php echo implode( ' ', $custom_attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> />
			</td>
		</tr>
		<?php
	}
	...
	public function sanitize_checkbox_multiple_field( $value ) {
		...
		$option_value = ( isset( $_POST[ $value['id'] ] ) ? wc_clean( wp_unslash( $_POST[ $value['id'] ] ) ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
						...
						<fieldset>
							<label for="<?php echo esc_attr( $value['id'] . '_' . $key ); ?>">
								<input
									...
									<?php checked( ( ( in_array( $key, $option_values ) || $value['default'] == $key ) ? true : false ), true ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict, WordPress.PHP.StrictComparisons.LooseComparison ?>
									<?php echo implode( ' ', $custom_attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		...
		<tr valign="top">
			<td class="forminp forminp-checkbox wc-gzdp-disclaimer notice notice-success">
				<div class="disclaimer-text">
					<h3>Disclaimer</h3>
					<?php echo wp_kses_post( html_entity_decode( $value['desc_tip'] ) ); ?>
				</div>
				<fieldset>
	...
	public function sanitize_select_page_field( $value ) {
		$option_value = ( isset( $_POST[ $value['id'] ] ) ? absint( $_POST[ $value['id'] ] ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		return $option_value;
	}
	...
	public function select_page_field( $value ) {
		...
		<tr valign="top" class="single_select_page">
			...
			<td class="forminp">
				<?php echo str_replace( ' id=', " data-placeholder='" . __( 'Select a page&hellip;', 'woocommerce-germanized-pro' ) . "' " . implode( ' ', $custom_attributes ) . " style='" . esc_attr( $value['css'] ) . "' class='" . esc_attr( $value['class'] ) . "' id=", wp_dropdown_pages( $args ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</td>
		</tr>
		<?php
	}
	...
	public function notice_field( $value ) {
		?>
		<tr valign="top">
			<td class="forminp wc-gzdp-notice notice notice-<?php echo esc_attr( isset( $value['notice_type'] ) ? $value['notice_type'] : 'warning' ); ?>">
				<?php echo ( isset( $value['custom_attributes']['data-custom-desc'] ) ? wp_kses_post( html_entity_decode( $value['custom_attributes']['data-custom-desc'] ) ) : '' ); ?>
			</td>
		</tr>
		<?php
	}
