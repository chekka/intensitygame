...
class WC_GZDP_VAT_Validation {
	...
	public function __construct( $options = array() ) {
		...
		$this->options['address_expected'] = wp_parse_args(
			$this->options['address_expected'],
			array(
				'company'  => '',
				'postcode' => '',
				'city'     => '',
		...
		if ( $this->client ) {
			try {
				...
				if ( $rs->valid ) {
					...
					if ( ! empty( $address ) ) {
						if ( ! empty( $company ) && in_array( 'company', $fields_to_check, true ) ) {
						...
						if ( ! empty( $city ) && in_array( 'city', $fields_to_check, true ) ) {
							$pattern = '/\b' . $city . '\b/u';
						...
						if ( ! empty( $postcode ) && in_array( 'postcode', $fields_to_check, true ) ) {
