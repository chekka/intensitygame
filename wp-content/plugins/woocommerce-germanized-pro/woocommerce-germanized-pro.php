		...
		<div id="message" class="error">
			<p>
				...
				printf(
					...
					esc_html__( 'Germanized Pro requires at least PHP 5.6 to work. Please %s your PHP version.', 'woocommerce-germanized-pro' ),
...
if ( ! class_exists( 'WooCommerce_Germanized_Pro' ) ) :
	...
	final class WooCommerce_Germanized_Pro {
		...
		public function __construct() {
			...
			register_deactivation_hook( __FILE__, array( 'WC_GZDP_Install', 'deactivate' ) );
		...
		public function hide_attachments( $query ) {
			...
			if ( $query->is_attachment || ( ! is_array( $post_type ) && 'attachment' === $post_type ) || ( is_array( $post_type ) && in_array( 'attachment', $post_type, true ) ) ) {
				$filter = true;
			}
		...
		public function action_links( $links ) {
			return array_merge(
				array(
					'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=germanized' ) ) . '">' . __( 'Settings', 'woocommerce-germanized-pro' ) . '</a>',
 ...
 */
function WC_germanized_pro() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
