...
function gr_create_help_page() {
    ...
    $apiStatus = Api::STATUS_DISCONNECTED;
    $apiStatusDetails = '';
    ...
    if ($api !== null) {
        ...
        try {
            $api->connect();
            $apiStatus = Api::STATUS_CONNECTED;
        } catch (ApiException $e) {
            $apiStatus = Api::STATUS_ERROR;
            $apiStatusDetails = $e->getMessage();
        }
    }
	...
	gr_load_template(
        ...
        array(
            'apiStatus' => $apiStatus,
            'apiErrorDetails' => $apiStatusDetails
        )
    );
}
