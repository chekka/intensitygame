 ...
 */
class ImagePreview extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function init() {
        ...
        $attr = \array_values(
            \array_unique(
                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::array_flatten(
                    \array_column(\array_values($cb->getTagAttributeMap()), 'attr')
                )
            )
        );
        $transformedAttr = \array_map(
            [
                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AttributesHelper::class,
                'transformAttribute'
            ],
            $attr
        );
        $this->attributesToCheckForUrl = \array_merge($attr, $transformedAttr);
        $this->htmlHeadThumbnailParser = new \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\HtmlHeadThumbnailParser();
    }
    /**
    ...
    public function blockedMatch($result, $matcher, $match) {
        ...
        if ($foundUrl === null) {
            $foundUrl = $this->findUrlForSpecialMatch($match);
        }
        if (!empty($foundUrl)) {
            $blockable = $result->getFirstBlocked();
            // Check if URL is already masked with the blocker ID
            if (\DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::startsWith($foundUrl, '{')) {
            ...
            if (
                ...
                $blockable->downloadImagePreviewFor($foundUrl, $result, $match)
            ) {
                // Set the HTML attribute for our blocked item, so our `afterSetup` callback can retrieve it
                // and send requests in bulk.
                $match->setAttribute(
                    self::HTML_ATTRIBUTE_TO_FETCH_URL_FROM,
                    \json_encode([
                        'blockable' => $blockable->getBlockerId(),
                        'url' => \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\Thumbnail::normalizeEmbedUrl(
                            $foundUrl
                        ),
                        // Allow to override aspect ratio from `width` and `height` HTML attributes
                        'aspectRatio' =>
                            \is_numeric($match->getAttribute('width')) && \is_numeric($match->getAttribute('height'))
                                ? (\intval($match->getAttribute('height')) / \intval($match->getAttribute('width'))) *
                                    100
                                : null
                    ])
                );
    ...
    protected function findUrlForSpecialMatch($match) {
        $tag = $match->getTag();
        $class = $match->getAttribute(
            \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AttributesHelper::transformAttribute('class'),
     ...
     */
    protected function isValidEmbedUrl($embedUrl) {
        if (empty($embedUrl)) {
            return \false;
        }
        // Check if the attribute is a JSON string
        if (\DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::startsWith($embedUrl, '{')) {
            $decoded = \html_entity_decode($embedUrl);
            $json = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::isJson($decoded);
            if (\is_array($json)) {
                $foundUrl = null;
        ...
        $urlToCheck = set_url_scheme($embedUrl, 'https');
        if (\filter_var($urlToCheck, \FILTER_VALIDATE_URL)) {
            $extension = \pathinfo(\parse_url($urlToCheck, \PHP_URL_PATH), \PATHINFO_EXTENSION);
        ...
        $this->getHeadlessContentBlocker()->addCallback(function ($html) {
            ...
            if (\preg_match_all($regex, $html, $urlsToFetch, \PREG_SET_ORDER, 0)) {
                ...
                foreach ($urlsToFetch as &$val) {
                    $val = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::parseHtmlAttributes($val);
                    $val = $val[self::HTML_ATTRIBUTE_TO_FETCH_URL_FROM] ?? null;
                    if ($val !== null) {
                        $val = \json_decode($val, ARRAY_A);
                        $blockerId = $val['blockable'];
                        $url = $val['url'];
                        if (\is_numeric($blockerId)) {
                            $blockable = $this->getHeadlessContentBlocker()->getBlockableById(\intval($blockerId));
                            if ($blockable !== null) {
                                $thumbnailInstance = new \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\Thumbnail(
                                    $url,
                                    $blockable
                                );
                ...
                return \preg_replace_callback(
                    $regex,
                    function ($m) use ($urlToThumbnail) {
                        $embedUrl = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::parseHtmlAttributes(
                            $m[0]
                        );
                        ...
                        $embedUrl = \json_decode($embedUrl, ARRAY_A)['url'];
                        /**
                         * Thumbnail
                         *
                         * @var Thumbnail
                        ...
                        if (
                            $thumbnail !== null &&
                            $thumbnail->getAllowance() !==
                                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\Thumbnail::ALLOWANCE_NO
                        ) {
                            $useAttribute =
                                $thumbnail->getAllowance() ===
                                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\Thumbnail::ALLOWANCE_SUGGEST
                                    ? \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_THUMBNAIL_SUGGESTION
                                    : \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_THUMBNAIL;
                            return \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::htmlAttributes([
                                $useAttribute => \json_encode($this->getWriteJson($thumbnail))
                            ]);
                        }
    ...
    public function getWriteJson($thumbnail) {
        $writeJson = [
            'embedId' => $thumbnail->getId(),
            'fileMd5' => $thumbnail->getMd5File(),
            'url' => $thumbnail->getCacheUrl(),
    ...
    public function fetchMetadata(&$thumbnails) {
        ...
        if (\count($notFoundEmbedUrls) > 0) {
            try {
                $embera = new \DevOwl\RealCookieBanner\Vendor\Embera\Embera();
