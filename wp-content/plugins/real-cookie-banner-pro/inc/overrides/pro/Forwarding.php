...
class Forwarding {
        ...
        if ($mu->isConsentForwarding() && !$request->get_param('markAsDoNotTrack')) {
            ...
            if (\count($endpoints) > 0) {
                $data = [
                    ...
                    'groups' => $this->groupIdsToUniqueName(\array_keys($result['decision'])),
                    'cookies' => $this->cookieIdsToUniqueName($result['decision']),
    ...
    public function getByUniqueName($slug) {
        // TODO: should we fill services with empty meta value with a slugified string?
        //       Currently there is a bug: Create a service -> enable Consent Forwarding ->
        //       create another service with the same unique name -> still allowed, but should be disabled?
        $result = [];
        ...
        $byMeta = get_posts(
            \DevOwl\RealCookieBanner\Core::getInstance()->queryArguments(
                [
                    ...
                    'meta_query' => [
                        [
                            'key' => \DevOwl\RealCookieBanner\settings\Cookie::META_NAME_CONSENT_FORWARDING_UNIQUE_NAME,
                            'value' => $slug,
                            'compare' => '='
                        ]
                    ]
