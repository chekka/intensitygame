 ...
 */
class WordPressImagePreviewCache extends
    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\FsImagePreviewCache {
    use UtilsProvider;
    /**
    ...
    public function allowance($thumbnails) {
        ...
        foreach ($thumbnails as $embedUrl => $thumbnail) {
            $userGaveConsentForThisThumbnail = \true;
            if ($userGaveConsentForThisThumbnail) {
                $thumbnail->setAllowance(
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\Thumbnail::ALLOWANCE_PUBLIC
                );
    ...
    public function set($thumbnails) {
        ...
        foreach ($thumbnails as $thumbnail) {
            ...
            $persistValues[] = $wpdb->prepare(
                '%s, %s, %s, %s, %s, %d, %d',
                $thumbnail->getId(),
                $thumbnail->getMd5File(),
                $thumbnail->getEmbedUrl(),
                \basename($thumbnail->getCacheUrl()),
                $thumbnail->getTitle() ?? 'NULL',
                $thumbnail->getWidth(),
                $thumbnail->getHeight()
            );
        ...
        foreach ($chunks as $sqlInsert) {
            $sql = \str_ireplace(
                "'NULL'",
                'NULL',
                "INSERT IGNORE INTO {$table_name} (embed_id, file_md5, embed_url, cache_filename, title, width, height) VALUES (" .
                    \implode('),(', $sqlInsert) .
                    ')'
            );
            // phpcs:disable WordPress.DB.PreparedSQL
    ...
    public static function create($uploadsSubfolder = 'embed-thumbnails') {
        ...
        $imagePreviewFolder = path_join($uploadDir['basedir'], $uploadsSubfolder);
        if (wp_mkdir_p($imagePreviewFolder) && wp_is_writable($imagePreviewFolder)) {
            try {
                return new \DevOwl\RealCookieBanner\lite\view\blocker\WordPressImagePreviewCache(
                    $imagePreviewFolder . '/',
                    $uploadDir['baseurl'] . '/' . $uploadsSubfolder . '/',
                    60 * 60 * 24 * 14
                );
            } catch (\Exception $e) {
                return \false;
