...
class SiteOrigin_Premium_Plugin_WooCommerce_Templates {
	...
	public function __construct() {
		...
		if ( class_exists( 'Black_Studio_TinyMCE_Admin' ) ) {
			add_filter( 'black_studio_tinymce_enable_pages', array( $this, 'compatibility_black_studio' ) );
		}

		// Add compatibility for the compatibility for WooCommerce PayPal Payments plugin. 
		if ( is_plugin_active( 'woocommerce-paypal-payments/woocommerce-paypal-payments.php' ) ) {
			add_action( 'parse_query', array( $this, 'compatibility_woocommerce_paypal_payments_setup' ) );
		}

		if ( function_exists( 'nm_gift_registry' ) ) {
			add_action( 'init', array( $this, 'compatibility_nm_gift_registry' ) );
		}
	}
	...
	public function render_template_builder() {
		...
		if ( ! class_exists( 'SiteOrigin_Widgets_Bundle' ) ) {
			?>
			<div class="so-wc-templates-missing-plugin"><p><strong><?php esc_html_e( 'Please install and activate the SiteOrigin Widgets Bundle plugin before using this addon.', 'siteorigin-premium' ) ?></strong></p></div>
			<?php
			return;
		}
	...
	public function get_template( $template ) {
		...
		if ( ! empty( $override ) ) {
			...
			// If this an archive, we need to determine if should be showign the before/after product data loop. 
	...
	function before_template_render() {
	...
	function remove_widget_class( $classes ) {
	...
	function after_template_render() {
	...
	function set_active_template( $custom_template_id, $template_data ) {
		...
		if ( ! empty( $_GET['siteorigin_premium_template_preview'] ) ) {
			...
			if ( empty( $template_post_id ) && ! empty( $template_data['post_id'] ) ) {
				// Attempt to use the default template.
				$template_post_id = ! empty( get_post( $template_data['post_id'] ) ) ? $template_data['post_id'] : 0;
			}
		}
	...
	function compatibility_black_studio( $enabled_pages ) {
		global $pagenow;
	...
	function compatibility_woocommerce_paypal_payments_setup() {
			...
			if ( ! is_admin() && get_query_var( 'post_type' ) == 'product' ) {
				...
				if ( ! empty( $override ) ) {
					add_filter(
						...
						array( $this, 'compatibility_woocommerce_paypal_payments' )
					);
				}
			}
		}
	...
	function compatibility_woocommerce_paypal_payments( $filter ) {
		return 'siteorigin_premium_addon_wctb_add_to_cart_after';
	}

	// NM Gift Registry and Wishlist compatibility.
	function compatibility_nm_gift_registry() {
		...
		if ( is_numeric( $single_product_position ) ) {
			switch ( $single_product_position ) {
				case 1: // Before title.
					$hook = 'siteorigin_premium_addon_wctb_single_product_title_before';
					break;
				case 6: // After title.
					$hook = 'siteorigin_premium_addon_wctb_single_product_title_after';
					break;
				case 15: // After price.
					$hook = 'siteorigin_premium_addon_wctb_single_product_price_after';
					break;
				case 25: // After excerpt.
					$hook = 'siteorigin_premium_addon_wctb_single_product_excerpt_after';
					break;
				case 35: // After add to cart button.
					$hook = 'siteorigin_premium_addon_wctb_add_to_cart_after';
					break;
				case 45: // After meta information.
					$hook = 'siteorigin_premium_addon_wctb_single_product_meta_after';
					break;
			}
