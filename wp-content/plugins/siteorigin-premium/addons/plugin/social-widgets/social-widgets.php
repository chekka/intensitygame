...
class SiteOrigin_Premium_Plugin_Social_Widgets {
	...
	function __construct() {
		add_filter( 'sow_social_media_buttons_form_options', array( $this, 'add_custom_network_form_options' ) );
	 ...
	 * @return array
	 */
	function add_custom_network_form_options( $form_options ) {
	 ...
	 * @return array
	 */
	function merge_custom_networks( $networks, $instance ) {
	 ...
	 * @return string
	 */
	function premium_template( $template_html, $instance, $widget ) {
		if ( empty( $instance['custom_networks'] ) ) {
	 ...
	 * @return string
	 */
	function premium_styles( $less, $instance, $widget ) {
		$less .= "
		a {
			.sow-icon-image {
				width: 1em;
